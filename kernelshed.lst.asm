...
0x00002824       LDW R8 [SP + TF_R1]        ; R8 = exit code

; macro: GET_CURR_TASK_IDX R2
0x00002828   LI R1 CURRENT_TASK
0x00002830   LDW R2 [R1]
; macro: GET_TASK_PTR R5, R2
0x00002834   LI R1 TASK_SIZE
0x0000283C   MUL R3 R2 R1
0x00002840   LI R5 tasks
0x00002848   ADD R5 R5 R3

    ; Store exit code in child task struct for parent to collect in waitforpid
; macro: TASK_SET_EXIT_CODE R5, R8  ; Save exit code
0x0000284C   STW R8 [R5 + TASK_EXIT_CODE]

0x00002850       PUSH R5
0x00002854       MOV R1 R5
0x00002858       BL task_close_fds          ; close all open file descriptors of this task (if any) to free file_pool resources
0x00002860       POP R5

    ; Mark this child as zombie (still exists but not runnable)
; macro: TASK_SET_STATE R5, TASK_ZOMBIE
0x00002864   LI R1 TASK_ZOMBIE
0x0000286C   STW R1 [R5 + TASK_STATE]
; macro: TASK_SET_WAIT R5, WAIT_NONE
0x00002870   LI R1 WAIT_NONE
0x00002878   STW R1 [R5 + TASK_WAIT]

    ; Wake parent if it's waiting
; macro: TASK_GET_PPID R6, R5       ; R6 = parent PID
0x0000287C   LDW R6 [R5 + TASK_PPID]

    ; find parent task by PPID
0x00002880       MOV R1 R6
0x00002884       LI R2 0                    ; Search by PID (parent's PID)
0x0000288C       BL task_find               ; R1 = found parent task*
0x00002894       CMP R1 0
0x00002898       BEQ no_parent_waiting
0x000028A0       MOV R7 R1                  ; R7 = parent task*
0x000028A4       MOV R11 R2                 ; save parent task index for bitmask

    ;Check if parent is waiting for this child
; macro: TASK_GET_WAIT_CHILD R8, R7 ; Child PID that parent R7 ptr is waiting for
0x000028A8   LDW R8 [R7 + TASK_WAIT_CHILD]
; macro: TASK_GET_PID R9, R5        ; This child's R5 ptr PID
0x000028AC   LDW R9 [R5 + TASK_PID]

0x000028B0       LI R10 -1
0x000028B8       CMP R8 R10                 ; if parent is waiting for any child (-1), then wake it up
0x000028BC       BEQ wake_parent            ;

0x000028C4       CMP R8 R9
0x000028C8       BNE no_parent_waiting      ; parent is waiting for a different child, do not wake it up
...
0x000028D0       LI R9 1
0x000028D8       SHL R9 R9 R11               ; bit for parent task

0x000028DC       LI R1 child_waitq
0x000028E4       MOV R2 R9
0x000028E8       BL waitq_wake_bitmask       ;unblock parent task waiting for this child

no_parent_waiting:
0x000028F0       B schedule_and_switch
...
syscall_waitpid:
0x000028F8       LDW R8 [SP + TF_R1]        ; R8 = pid to wait for
0x000028FC       LDW R9 [SP + TF_R2]        ; R9 = status pointer

    ; Validate status pointer
0x00002900       CMP R9 0
0x00002904       BEQ waitpid_validate_done
0x0000290C       MOV R1 R9
0x00002910       LI R2 4
0x00002918       LI R3 1
0x00002920       BL user_buffer_valid_range
0x00002928       CMP R1 1
0x0000292C       BNE waitpid_badptr
...
0x00002958   LDW R10 [R5 + TASK_PID]

    ; if search for any child
0x0000295C       LI  R2 -1
0x00002964       CMP R8 R2
0x00002968       BNE find_child_by_pid
    ; set task_find to search for any child of this parent
0x00002970       MOV R1 R10                  ; R1 = parent PID (PPID in child task)
0x00002974       LI  R2 1                    ; search by PPID
0x0000297C       BL task_find               ; R1 = found child task*
0x00002984       CMP R1 0
0x00002988       BEQ waitpid_no_child        ; No any child with PPID = this parent PID found
    ;R1 child task* found
0x00002990       B find_any_child_found
find_child_by_pid:
    ; Search for child task by PID
0x00002998       MOV R1 R8                  ; R1 = child PID to search for
0x0000299C       LI R2 0                    ; Search by PID
0x000029A4       BL task_find               ; R1 = found child task*
0x000029AC       CMP R1 0
0x000029B0       BEQ waitpid_no_child        ; No such child

find_any_child_found:

0x000029B8       MOV R7 R1                   ; R7 = child task*

    ; Verify it's actually our child by its PPID fld
; macro: TASK_GET_PPID R1, R7
0x000029BC   LDW R1 [R7 + TASK_PPID]
0x000029C0       CMP R1 R10
0x000029C4       BNE waitpid_no_child
    ; R7 = child task*
    ; check its state, if ZOMBIE, we can reap it and return its exit code
; macro: TASK_GET_STATE R1, R7
0x000029CC   LDW R1 [R7 + TASK_STATE]
0x000029D0       CMP R1 TASK_ZOMBIE
0x000029D4       BEQ waitpid_reap_child

    ; Child running - block parent
; macro: TASK_SET_WAIT_CHILD R5, R12
0x000029DC   STW R12 [R5 + TASK_WAIT_CHILD]

0x000029E0       LI R1 child_waitq           ; child_waitq ptr
0x000029E8       LI R2 WAIT_CHILD            ; reason
0x000029F0       LI R3 TASK_SLEEPING         ; state to set for current task
0x000029F8       BL waitq_prepare_sleep

0x00002A00       BL waitq_sleep_current     ; freeze the current task
...
0x00002A08   LDW R2 [R7 + TASK_EXIT_CODE]

    ; If status pointer is not NULL, write exit code to user space
0x00002A0C       CMP R9 0
0x00002A10       BEQ waitpid_reap_done

0x00002A18       MOV R1 R9                  ; R1 = user status pointer
0x00002A1C       MOV R2 R2                  ; R2 = exit code
0x00002A20       MOV R3 4                   ; R3 = size of exit code
0x00002A24       BL copy_to_user            ; write exit code to user space

waitpid_reap_done:
; macro: TASK_GET_PID R10, R7       ; get child's PID
0x00002A2C   LDW R10 [R7 + TASK_PID]
0x00002A30       MOV R1 R7                  ; R1 = child task*
0x00002A34       BL task_destroy

0x00002A3C       STW R10 [SP + TF_R1]        ; save child's PID to trapframe for return
0x00002A40       B trap_restore

waitpid_no_child:
0x00002A48       LI R1 ERR_CHILD
0x00002A50       STW R1 [SP + TF_R1]
0x00002A54       B trap_restore

waitpid_badptr:
0x00002A5C       LI R1 ERR_FAULT
0x00002A64       STW R1 [SP + TF_R1]
0x00002A68       B trap_restore
...
0x00002A78       MOV R5 R2                  ; Save search mode
0x00002A7C       LI R2 0                    ; Task index
0x00002A84       LI R3 MAX_TASKS

task_find_loop:
0x00002A8C       CMP R2 R3
0x00002A90       BGE task_find_not_found

; macro: GET_TASK_PTR R4, R2
0x00002A98   LI R1 TASK_SIZE
0x00002AA0   MUL R3 R2 R1
0x00002AA4   LI R4 tasks
0x00002AAC   ADD R4 R4 R3
; macro: TASK_GET_STATE R6, R4
0x00002AB0   LDW R6 [R4 + TASK_STATE]
0x00002AB4       CMP R6 TASK_DEAD
0x00002AB8       BEQ task_find_next         ; Skip dead tasks

    ; Search based on mode
0x00002AC0       CMP R5 0
0x00002AC4       BEQ task_find_by_pid

    ; Search by PPID
; macro: TASK_GET_PPID R6, R4
0x00002ACC   LDW R6 [R4 + TASK_PPID]
0x00002AD0       CMP R6 R1
0x00002AD4       BEQ task_find_found
0x00002ADC       B task_find_next

task_find_by_pid:
; macro: TASK_GET_PID R6, R4
0x00002AE4   LDW R6 [R4 + TASK_PID]
0x00002AE8       CMP R6 R1
0x00002AEC       BEQ task_find_found

task_find_next:
0x00002AF4       ADD R2 R2 1
0x00002AF8       B task_find_loop

task_find_found:
0x00002B00       MOV R1 R4                  ; Return task pointer
0x00002B04       MOV R2 R2                  ; Return task index
0x00002B08       POP R6
0x00002B0C       POP R5
0x00002B10       RET
...
; macro: GET_CURR_TASK_IDX R2
0x00002B28   LI R1 CURRENT_TASK
0x00002B30   LDW R2 [R1]
; macro: GET_TASK_PTR R5, R2
0x00002B34   LI R1 TASK_SIZE
0x00002B3C   MUL R3 R2 R1
0x00002B40   LI R5 tasks
0x00002B48   ADD R5 R5 R3
; macro: TASK_GET_PID R1, R5            ; get pid from task scheduler data
0x00002B4C   LDW R1 [R5 + TASK_PID]

0x00002B50       STW R1 [SP + TF_R1]           ; save it to its trapframe which goes back when it s next time this task resumes
...
0x00002B70       LDW R2 [SP + TF_R2]

0x00002B74       BL copy_path_from_user     ; macro inside destroys R11, copy pathname
                               ; to tasks Kbuf_RD buffer
                               ; R1 - pathname str ptr in the bufer
0x00002B7C       CMP R1 0
0x00002B80       BEQ open_fail_fault

    ; copy_path_from_user returned the current task's kernel read buffer.
; macro: GET_CURR_TASK_IDX R4
0x00002B88   LI R1 CURRENT_TASK
0x00002B90   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00002B94   LI R1 TASK_SIZE
0x00002B9C   MUL R3 R4 R1
0x00002BA0   LI R5 tasks
0x00002BA8   ADD R5 R5 R3
; macro: TASK_GET_KBUF_RD R1, R5
0x00002BAC   LDW R1 [R5 + TASK_KBUF_RD_PTR]

0x00002BB0       BL vfs_open

0x00002BB8       STW R1 [SP + TF_R1]     ;file opened if fd on exit!
0x00002BBC       B trap_restore

open_fail_fault:
0x00002BC4       LI R1 ERR_FAULT
0x00002BCC       STW R1 [SP + TF_R1]     ;file not opened ERR
...
0x00002BD8       LDW R8 [SP + TF_R1]        ; R8 = milliseconds

0x00002BDC       CMP R8 0
0x00002BE0       BLE sleep_invalid          ; must be positive

; macro: GET_CURR_TASK_IDX R4
0x00002BE8   LI R1 CURRENT_TASK
0x00002BF0   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00002BF4   LI R1 TASK_SIZE
0x00002BFC   MUL R3 R4 R1
0x00002C00   LI R5 tasks
0x00002C08   ADD R5 R5 R3

    ; Calculate wake time = current time + requested ms
0x00002C0C       LI R3 timer_ticks
0x00002C14       LDW R6 [R3]                ; current ticks (2ms per tick)

    ; Convert ms to ticks: ms / 2
0x00002C18       SHR R7 R8 1                ; R7 = ticks to sleep

0x00002C1C       ADD R6 R6 R7               ; R6 = wake time in ticks

    ; Store wake time in task struct
; macro: TASK_SET_WAKE_TIME R5, R6
0x00002C20   STW R6 [R5 + TASK_WAKE_TIME]

    ; Use existing wait queue infrastructure
0x00002C24       LI R1 sleep_waitq           ; sleep_waitq ptr
0x00002C2C       LI R2 WAIT_SLEEP            ; reason
0x00002C34       LI R3 TASK_SLEEPING         ; new state (if other then blocked_io)
0x00002C3C       BL waitq_prepare_sleep     ; This marks task as TASK_SLEEP and adds it to the sleep_waitq

0x00002C44       BL waitq_sleep_current     ; freeze the current task in kernel side until it is woken up by the timer interrupt handler when the wake time is reached

    ; Return 0 (will be set when woken)
0x00002C4C       LI R1 0
0x00002C54       STW R1 [SP + TF_R1]
0x00002C58       B trap_restore

sleep_invalid:
0x00002C60       LI R1 ERR_FAULT
0x00002C68       STW R1 [SP + TF_R1]
0x00002C6C       B trap_restore
...
0x00002C78       MOV R8 R1                  ; current user source byte

; macro: GET_CURR_TASK_IDX R4
0x00002C7C   LI R1 CURRENT_TASK
0x00002C84   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00002C88   LI R1 TASK_SIZE
0x00002C90   MUL R3 R4 R1
0x00002C94   LI R5 tasks
0x00002C9C   ADD R5 R5 R3
; macro: TASK_GET_KBUF_RD R9, R5    ; destination kernel path buffer
0x00002CA0   LDW R9 [R5 + TASK_KBUF_RD_PTR]

0x00002CA4       PUSH R9                    ; original destination returned on success
0x00002CA8       LI R10 0                   ; bytes copied before NUL

copy_path_loop:
0x00002CB0       LI R11 KBUFFER_SIZE
0x00002CB8       CMP R10 R11
0x00002CBC       BGE copy_path_fail

0x00002CC4       PUSH R8
0x00002CC8       PUSH R9
0x00002CCC       PUSH R10
0x00002CD0       MOV R1 R8
0x00002CD4       LI R2 1
0x00002CDC       LI R3 0                    ; read access from user source
0x00002CE4       BL user_buffer_valid_range
0x00002CEC       POP R10
0x00002CF0       POP R9
0x00002CF4       POP R8
0x00002CF8       CMP R1 1
0x00002CFC       BNE copy_path_fail

0x00002D04       LDB R4 [R8]
0x00002D08       STB R4 [R9]
0x00002D0C       CMP R4 0
0x00002D10       BEQ copy_path_done

0x00002D18       ADD R8 R8 1
0x00002D1C       ADD R9 R9 1
0x00002D20       ADD R10 R10 1
0x00002D24       B copy_path_loop

copy_path_done:
0x00002D2C       POP R1                     ; original kernel path pointer
0x00002D30       POP LR
0x00002D34       RET

copy_path_fail:
0x00002D38       POP R1                     ; discard original kernel path pointer
0x00002D3C       LI R1 0
0x00002D44       POP LR
0x00002D48       RET
...
devfs_lookup:
0x00002D4C       PUSH LR
0x00002D50       MOV R8 R1                  ; save pathname ptr

0x00002D54       LI R7 device_table
0x00002D5C       LI R9 DEVICE_COUNT

devfs_loop:
0x00002D64       CMP R9 0
0x00002D68       BEQ lookup_fail

    ; compare pathname with device name
0x00002D70       MOV R1 R8
0x00002D74       LDW R2 [R7 + DEV_NAME]
0x00002D78       BL strcmp
0x00002D80       CMP R1 1
0x00002D84       BEQ devfs_found

0x00002D8C       ADD R7 R7 DEV_SIZE
0x00002D90       SUB R9 R9 1
0x00002D94       B devfs_loop

devfs_found:
    ; 1 allocate inode
0x00002D9C       BL inode_alloc
0x00002DA4       CMP R1 0
0x00002DA8       BEQ devfs_fail

0x00002DB0       MOV R10 R1         ; inode
    ; 2 init inode
0x00002DB4       LDW R2 [R7 + DEV_OPS]
0x00002DB8       LDW R3 [R7 + DEV_PRIVATE]
0x00002DBC       LI  R4 INODE_CHAR       ; inode type for dev - char
0x00002DC4       LI  R5 0                ; size =0
0x00002DCC       BL inode_init

0x00002DD4       MOV R1 R10         ; 3 return new inited inode ptr for this dev
0x00002DD8       POP LR
0x00002DDC       RET
...
0x00002DF4       MOV R8 R1                  ; save pathname ptr

0x00002DF8       LI R7 device_table
0x00002E00       LI R9 DEVICE_COUNT

lookup_loop:
0x00002E08       CMP R9 0
0x00002E0C       BEQ lookup_fail

    ; compare pathname with device name

0x00002E14       MOV R1 R8
0x00002E18       LDW R2 [R7 + DEV_NAME]

0x00002E1C       BL strcmp

0x00002E24       CMP R1 1
0x00002E28       BEQ lookup_found

0x00002E30       ADD R7 R7 DEV_SIZE
0x00002E34       SUB R9 R9 1
0x00002E38       B lookup_loop

lookup_found:

0x00002E40       MOV R1 R7                  ; return device descriptor ptr
...
str_loop:
0x00002E5C       LDB R3 [R1]
0x00002E60       LDB R4 [R2]

0x00002E64       CMP R3 R4
0x00002E68       BNE str_not_equal

0x00002E70       CMP R3 0
0x00002E74       BEQ str_equal

0x00002E7C       ADD R1 R1 1
0x00002E80       ADD R2 R2 1
0x00002E84       B str_loop
...
sp_loop:
0x00002EAC       LDB R3 [R2]            ; prefix char
0x00002EB0       CMP R3 0
0x00002EB4       BEQ sp_match           ; reached end of prefix?

0x00002EBC       LDB R4 [R1]            ; string char
0x00002EC0       CMP R4 R3
0x00002EC4       BNE sp_nomatch

0x00002ECC       ADD R1 R1 1
0x00002ED0       ADD R2 R2 1
0x00002ED4       B sp_loop
sp_match:
0x00002EDC       LI R1 1                 ;prefix ok
0x00002EE4       POP R4
0x00002EE8       POP R3
0x00002EEC       RET
sp_nomatch:
0x00002EF0       LI R1 0                 ; not ok
0x00002EF8       POP R4
0x00002EFC       POP R3
0x00002F00       RET
...
0x00002F08       PUSH R4
sk_loop:
0x00002F0C       LDB R3 [R2]            ; prefix char
0x00002F10       CMP R3 0
0x00002F14       BEQ sk_match           ; reached end of prefix
0x00002F1C       LDB R4 [R1]            ; string char
0x00002F20       CMP R4 R3
0x00002F24       BNE sk_nomatch
0x00002F2C       ADD R1 R1 1
0x00002F30       ADD R2 R2 1
0x00002F34       B sk_loop
...
sk_nomatch:
0x00002F48       LI R1 0                 ; no prefix/or prefix not matching with that in src string
0x00002F50       POP R4
0x00002F54       POP R3
0x00002F58       RET
...
0x00002F5C       PUSH R2
0x00002F60       PUSH R3
0x00002F64       LI R2 0                ; length
pcl_loop:
0x00002F6C       LDB R3 [R1]
0x00002F70       CMP R3 0
0x00002F74       BEQ pcl_done
0x00002F7C       LI R4 47               ; '/'
0x00002F84       CMP R3 R4
0x00002F88       BEQ pcl_done
0x00002F90       ADD R2 R2 1
0x00002F94       ADD R1 R1 1
0x00002F98       B pcl_loop
pcl_done:
0x00002FA0       MOV R1 R2
0x00002FA4       POP R3
0x00002FA8       POP R2
0x00002FAC       RET
...
file_init:
    ; file->inode = inode
0x00002FB0       STW R2 [R1 + FILE_INODE]
    ; file->offset = 0
0x00002FB4       LI R4 0
0x00002FBC       STW R4 [R1 + FILE_OFFSET]
    ; file->flags = O_RDONLY etc
0x00002FC0       STW R3 [R1 + FILE_FLAGS]
     ; file->refcnt = 1
0x00002FC4       LI R4 1
0x00002FCC       STW R4 [R1 + FILE_REFCNT]
0x00002FD0       RET
...
0x00002FD4       MOV R8 R1                  ; save file pointer

; macro: GET_CURR_TASK_IDX R4
0x00002FD8   LI R1 CURRENT_TASK
0x00002FE0   LDW R4 [R1]
; macro: GET_TASK_PTR R4, R4
0x00002FE4   LI R1 TASK_SIZE
0x00002FEC   MUL R3 R4 R1
0x00002FF0   LI R4 tasks
0x00002FF8   ADD R4 R4 R3
; macro: TASK_GET_FD_TABLE R4, R4   ; R4 = fd table ptr
0x00002FFC   LDW R4 [R4 + TASK_FD_TABLE]

0x00003000       LI R5 3                    ; start after stdin/out/err dynamic space

fd_alloc_loop:

0x00003008       CMP R5 MAX_FDS
0x0000300C       BGE fd_alloc_fail

0x00003014       SHL R6 R5 2                ; fd * 4
0x00003018       ADD R7 R4 R6               ; &fd_table[fd]

0x0000301C       LDW R2 [R7]
0x00003020       CMP R2 0                   ; 0 - empty
0x00003024       BEQ fd_alloc_found

0x0000302C       ADD R5 R5 1
0x00003030       B fd_alloc_loop

fd_alloc_found:

0x00003038       STW R8 [R7]                ; fd_table[fd] = file*

0x0000303C       MOV R1 R5                  ; return fd
0x00003040       RET

fd_alloc_fail:

0x00003044       LI R1 ERR_MFILE
0x0000304C       RET
...
syscall_close:
    ...
    ; out R1 = 0 / err -1
    ;================================================================
0x00003050       LDW R1 [SP + TF_R1]

0x00003054       BL vfs_close

0x0000305C       LI R1 0
0x00003064       STW R1 [SP + TF_R1]

0x00003068       B trap_restore
...
syscall_pipe:
    ...
    ; user int fd[2]
0x00003070       LDW R7 [SP + TF_R1]

0x00003074       BL pipe_alloc       ;create new pipe object in pipe_pool
0x0000307C       CMP R1 0
0x00003080       BEQ pipe_fail_nospc

0x00003088       MOV R8 R1            ; new slot in pipe_pool ( pipe* )
    ; [0] read end          write[1]>--pipe--->read[0]
0x0000308C       BL file_alloc        ; R1 - created read file ptr for read end
0x00003094       CMP R1 0
0x00003098       BEQ pipe_fail_read_fd

0x000030A0       MOV R9 R1           ; new file for read end  in file_pool
0x000030A4       BL inode_alloc      ; get inode for this end file
0x000030AC       CMP R1 0
0x000030B0       BEQ pipe_fail_ia_read_fd
0x000030B8       MOV R10 R1

0x000030BC       LI  R2 pipe_ops         ; pipe_ops table
0x000030C4       MOV R3 R8               ; store our slot pipe*
0x000030C8       LI  R4 INODE_PIPE       ; inode type PIPE
0x000030D0       LI  R5 0                ; size =0
0x000030D8       BL inode_init           ; make inode for read end

    ; initialize file object ;read end file
0x000030E0       MOV R1 R9                ; R1 file*
0x000030E4       MOV R2 R10               ; inode*
0x000030E8       LI R3  FD_FLAG_READ      ; flags READ end
0x000030F0       BL file_init

0x000030F8       MOV R1 R9
0x000030FC       BL fd_alloc                 ; insert read file to fd_table of user process

0x00003104       LI R2 ERR_MFILE             ; check if fd_alloc problem
0x0000310C       CMP R1 R2
0x00003110       BEQ pipe_fail_read_file

0x00003118       MOV R12 R1           ; get file read fd created to R10

    ; same for write end
0x0000311C       BL file_alloc
0x00003124       CMP R1 0
0x00003128       BEQ pipe_fail_ia_write_fd
0x00003130       MOV R9 R1

0x00003134       BL inode_alloc      ; get inode for this end file
0x0000313C       CMP R1 0
0x00003140       BEQ pipe_fail_ia_write_fd
0x00003148       MOV R10 R1

0x0000314C       LI  R2 pipe_ops         ; pipe_ops table
0x00003154       MOV R3 R8               ; store our slot pipe* need to check if this is ok here (might be changed)
0x00003158       LI  R4 INODE_PIPE       ; inode type PIPE
0x00003160       LI  R5 0                ; size =0
0x00003168       BL inode_init           ; make inode for write end

    ; initialize file object ;write end file
0x00003170       MOV R1 R9                ; R1 file*
0x00003174       MOV R2 R10               ; inode*
0x00003178       LI  R3 FD_FLAG_WRITE     ; flags WRITE end
0x00003180       BL file_init

0x00003188       MOV R1 R9
0x0000318C       BL  fd_alloc

0x00003194       LI  R2 ERR_MFILE         ; check if fd_alloc problem
0x0000319C       CMP R1 R2
0x000031A0       BEQ pipe_fail_write_file

0x000031A8       MOV R11 R1           ; R11 is write and fd R12 is read fd

0x000031AC       MOV R1 R7    ; in &fd[2]. not sure if R7 still has value for this ptr
0x000031B0       LI  R2 8     ; len 2 words (8 bytes)
0x000031B8       LI  R3 1     ; mem perm to write cond
0x000031C0       BL  user_buffer_valid_range
0x000031C8       CMP R1 1
0x000031CC       BNE pipe_fail_both_fds

0x000031D4       STW R12 [R7]     ;fill fd user array of read and write ends fd[0]-rd fd[1]-wr
0x000031D8       STW R11 [R7 + 4]

0x000031DC       LI R1 0
0x000031E4       STW R1 [SP + TF_R1]

0x000031E8       B trap_restore

pipe_fail:
0x000031F0       LI R1 ERR_IO
0x000031F8       STW R1 [SP + TF_R1]

0x000031FC       B trap_restore

pipe_fail_both_fds:
0x00003204       MOV R12 R8
0x00003208       MOV R1 R11
0x0000320C       BL fd_remove
0x00003214       CMP R1 0
0x00003218       BEQ pipe_fail_both_fds_read
0x00003220       BL file_free

pipe_fail_both_fds_read:
0x00003228       MOV R1 R10
0x0000322C       BL fd_remove
0x00003234       CMP R1 0
0x00003238       BEQ pipe_fail_free_pipe_fault
0x00003240       BL file_free

pipe_fail_free_pipe_fault:
0x00003248       MOV R1 R12
0x0000324C       BL pipe_free
...
0x00003268       MOV R12 R8
0x0000326C       MOV R1 R9
0x00003270       BL file_free
0x00003278       MOV R1 R10
0x0000327C       BL fd_remove
0x00003284       CMP R1 0
0x00003288       BEQ pipe_fail_free_pipe_mfile
0x00003290       BL file_free

pipe_fail_free_pipe_mfile:
0x00003298       MOV R1 R12
0x0000329C       BL pipe_free
0x000032A4       LI R1 ERR_MFILE
0x000032AC       STW R1 [SP + TF_R1]

0x000032B0       B trap_restore

pipe_fail_read_fd:
0x000032B8       MOV R12 R8
0x000032BC       MOV R1 R10
0x000032C0       BL fd_remove
0x000032C8       CMP R1 0
0x000032CC       BEQ pipe_fail_free_pipe_nfile
0x000032D4       BL file_free

pipe_fail_free_pipe_nfile:
0x000032DC       MOV R1 R12
0x000032E0       BL pipe_free
0x000032E8       LI R1 ERR_NFILE
...
0x00003304       BL file_free
0x0000330C       MOV R1 R10          ; освободить inode read end
0x00003310       BL inode_free
0x00003318       MOV R1 R12
0x0000331C       BL pipe_free
0x00003324       LI R1 ERR_MFILE
0x0000332C       STW R1 [SP + TF_R1]

0x00003330       B trap_restore

pipe_fail_pipe_only:
0x00003338       MOV R1 R8
0x0000333C       BL pipe_free
0x00003344       LI R1 ERR_NFILE
0x0000334C       STW R1 [SP + TF_R1]

0x00003350       B trap_restore

pipe_fail_nospc:
0x00003358       LI R1 ERR_NOSPC
0x00003360       STW R1 [SP + TF_R1]

0x00003364       B trap_restore

pipe_fail_ia_read_fd:
    ; Ошибка при создании inode для read end
0x0000336C       MOV R1 R9          ; освобождаем file (read end)
0x00003370       BL  file_free
0x00003378       MOV R1 R8          ; освобождаем pipe
0x0000337C       BL  pipe_free
0x00003384       LI R1 ERR_NFILE    ; или ERR_NOMEM - смотрите ваши коды ошибок
0x0000338C       STW R1 [SP + TF_R1]
0x00003390       B trap_restore

pipe_fail_ia_write_fd:
    ; Ошибка при создании inode для write end
0x00003398       MOV R1 R12         ; освобождаем read fd (если уже создан)
0x0000339C       BL fd_remove
0x000033A4       CMP R1 0
0x000033A8       BEQ skip_file_free_read
0x000033B0       BL file_free
skip_file_free_read:
0x000033B8       MOV R1 R9          ; освобождаем file (write end)
0x000033BC       BL file_free
0x000033C4       MOV R1 R8          ; освобождаем pipe
0x000033C8       BL pipe_free
0x000033D0       LI R1 ERR_NFILE
0x000033D8       STW R1 [SP + TF_R1]
0x000033DC       B trap_restore
...
0x000033E4       LDW R1 [SP + TF_R1]     ; argument fd

0x000033E8       BL fd_lookup            ; lookup FILE*
0x000033F0       CMP R1 0
0x000033F4       BEQ dup_badfd
0x000033FC       MOV R8 R1               ; keep FILE*

0x00003400       BL file_get             ; FILE.ref++

0x00003408       MOV R1 R8
0x0000340C       BL fd_alloc             ; try to allocate new fd

0x00003414       LI R2 ERR_MFILE
0x0000341C       CMP R1 R2
0x00003420       BEQ dup_fail_fd

0x00003428       STW R1 [SP + TF_R1] ;R1 - new fd
0x0000342C       B trap_restore

dup_fail_fd:

0x00003434       MOV R1 R8
0x00003438       BL file_put

0x00003440       LI R1 ERR_MFILE     ;R1 -err + rollback
0x00003448       STW R1 [SP + TF_R1]
0x0000344C       B trap_restore

dup_badfd:

0x00003454       LI R1 ERR_BADF      ;R1 -err + file not found
0x0000345C       STW R1 [SP + TF_R1]

0x00003460       B trap_restore
...
0x00003468       LDW R8 [SP + TF_R1]         ; user pointer to struct timeval

    ;----------------------------------------------------------
    ; Validate destination buffer
    ;----------------------------------------------------------

0x0000346C       MOV R1 R8
0x00003470       LI  R2 TIMEVAL_SIZE
0x00003478       LI  R3 1                   ; write access
0x00003480       BL  user_buffer_valid_range

0x00003488       CMP R1 1
0x0000348C       BNE gettime_badptr

    ;----------------------------------------------------------
    ; Get current kernel time
    ;----------------------------------------------------------

0x00003494       BL clock_gettime           ;out: R1=sec, R2=usec
...
; macro: GET_CURR_TASK_IDX R4
0x0000349C   LI R1 CURRENT_TASK
0x000034A4   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x000034A8   LI R1 TASK_SIZE
0x000034B0   MUL R3 R4 R1
0x000034B4   LI R5 tasks
0x000034BC   ADD R5 R5 R3
; macro: TASK_GET_KBUF_WR R6, R5   ; R6 ptr kbuf_wr
0x000034C0   LDW R6 [R5 + TASK_KBUF_WR_PTR]

0x000034C4       STW R1 [R6 + TIMEVAL_SEC]
0x000034C8       STW R2 [R6 + TIMEVAL_USEC]

    ;----------------------------------------------------------
    ; Copy to user
    ;----------------------------------------------------------

0x000034CC       MOV R1 R6                  ; kernel source
0x000034D0       MOV R2 R8                  ; user destination
0x000034D4       LI  R3 TIMEVAL_SIZE        ; size in bytes (8)

0x000034DC       BL copy_to_user

0x000034E4       CMP R1 TIMEVAL_SIZE
0x000034E8       BNE gettime_badptr
...
0x000034F0       LI R1 0
0x000034F8       STW R1 [SP + TF_R1]

0x000034FC       B trap_restore

gettime_badptr:

0x00003504       LI R1 ERR_FAULT
0x0000350C       STW R1 [SP + TF_R1]

0x00003510       B trap_restore
...
syscall_brk:
0x00003518       LDW R8 [SP + TF_R1]        ; R8 = new break address (user space VA)

    ; Validate the address is within the data page
0x0000351C       LI R2 HEAP_START
0x00003524       CMP R8 R2
0x00003528       BLT brk_invalid            ; if new break is below data page, return error

0x00003530       LI R2 HEAP_END
0x00003538       CMP R8 R2
0x0000353C       BGT brk_invalid            ; if new break is above last address in data page, return error

    ; Get current task
; macro: GET_CURR_TASK_IDX R4
0x00003544   LI R1 CURRENT_TASK
0x0000354C   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00003550   LI R1 TASK_SIZE
0x00003558   MUL R3 R4 R1
0x0000355C   LI R5 tasks
0x00003564   ADD R5 R5 R3

    ; Set new break in task struct
    ; (We'll add this field to TASK structure)
; macro: TASK_SET_BREAK R5, R8
0x00003568   STW R8 [R5 + TASK_BREAK]

    ; Return new break
0x0000356C       STW R8 [SP + TF_R1]

0x00003570       B trap_restore

brk_invalid:
    ; Return -1
0x00003578       LI R1 ERR_FAULT
...
syscall_sbrk:
0x0000358C       LDW R8 [SP + TF_R1]        ; R8 = increment

    ; Get current task
; macro: GET_CURR_TASK_IDX R4
0x00003590   LI R1 CURRENT_TASK
0x00003598   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x0000359C   LI R1 TASK_SIZE
0x000035A4   MUL R3 R4 R1
0x000035A8   LI R5 tasks
0x000035B0   ADD R5 R5 R3

    ; Get current break
; macro: TASK_GET_BREAK R9, R5
0x000035B4   LDW R9 [R5 + TASK_BREAK]

    ; Calculate new break
0x000035B8       ADD R10 R9 R8

    ; Validate it's within the data page
0x000035BC       LI R2 HEAP_START
0x000035C4       CMP R10 R2
0x000035C8       BLT sbrk_invalid

0x000035D0       LI R2 HEAP_END
0x000035D8       CMP R10 R2
0x000035DC       BGT sbrk_invalid

    ; Return old break
0x000035E4       STW R9 [SP + TF_R1]     ; old break address

    ; Update break
; macro: TASK_SET_BREAK R5, R10  ;R10 - updated break address
0x000035E8   STW R10 [R5 + TASK_BREAK]

0x000035EC       B trap_restore

sbrk_invalid:
    ; Return -1
0x000035F4       LI R1 ERR_FAULT
0x000035FC       STW R1 [SP + TF_R1]
0x00003600       B trap_restore
...
0x00003608       LI  R3 timer_ticks
0x00003610       LDW R4 [R3]                ; tick counter (2 ms per tick)

    ; seconds = ticks / 500
0x00003614       MOV R1 R4
0x00003618       LI  R5 500
0x00003620       DIV R1 R1 R5

    ; usec = (ticks % 500) * 2000
0x00003624       MOD R4 R4 R5
0x00003628       LI  R5 2000
0x00003630       MUL R2 R4 R5
...
0x0000363C       MOV R9 R1              ; file*
0x00003640       MOV R7 R2              ; user buffer
0x00003644       MOV R6 R3              ; requested len

0x00003648       LDW R9 [R9 + FILE_INODE]
0x0000364C       LDW R9 [R9 + INODE_PRIVATE] ;get our Pipe instance allocated in pipe_pool (pipe*) (from its inode)
0x00003650       CMP R6 0                ;fast clear from it if len=0
0x00003654       BEQ pipe_read_done
;-----------------------------------------
...
0x00003664       MOV R1 R7
0x00003668       MOV R2 R6
0x0000366C       LI  R3 1               ; write access
0x00003674       BL user_buffer_valid_range

0x0000367C       POP R6
0x00003680       POP R7
0x00003684       CMP R1 1
0x00003688       BNE pipe_read_badptr

pipe_read_retry:
;-----------------------------------------
; anything in pipe?
;-----------------------------------------
0x00003690       LDW R4 [R9 + PIPE_COUNT]
0x00003694       CMP R4 0
0x00003698       BEQ pipe_read_sleep     ;go to sleep
;-----------------------------------------
; bytes_to_read=min(len (R6),count(R4)
;-----------------------------------------
0x000036A0       CMP R6 R4
0x000036A4       BLT pipe_user_len

0x000036AC       MOV R5 R4
0x000036B0       B pipe_have_amount

pipe_user_len:
0x000036B8       MOV R5 R6

pipe_have_amount:
0x000036BC       LI R10 0              ; bytes copied

pipe_read_loop:         ;cpy pipe_buffer to user with min(pipe_count,len) bytes
0x000036C4       CMP R10 R5
0x000036C8       BGE pipe_read_done

;------------------------------------------
; tail = pipe->tail (idx in PIPE_BUFFER in pipe*(R9) struc)
;------------------------------------------
0x000036D0       LDW R11 [R9 + PIPE_TAIL]
;------------------------------------------
; R12 addr = pipe + PIPE_BUFFER
;------------------------------------------
0x000036D4       MOV R12 R9
0x000036D8       ADD R12 R12 PIPE_BUFFER
0x000036DC       ADD R12 R12 R11         ; addr += tail

0x000036E0       LDB R4 [R12]    ;read data from buffer[tail_idx]

;------------------------------------------
; useraddr=userbuf+copied
;------------------------------------------
0x000036E4       MOV R12 R7
0x000036E8       ADD R12 R12 R10

0x000036EC       STB R4 [R12]    ;copy to user side

;------------------------------------------
    ; tail=(tail+1)&255
;------------------------------------------
0x000036F0       ADD R11 R11 1   ;update tail inc idx if idx > 255 idx=0
0x000036F4       LI R2 255
0x000036FC       AND R11 R11 R2
0x00003700       STW R11 [R9 + PIPE_TAIL]    ;save to pipe struc updated tail_idx
;------------------------------------------
; count-- (update to struc)
;------------------------------------------
0x00003704       LDW R12 [R9 + PIPE_COUNT]
0x00003708       SUB R12 R12 1
0x0000370C       STW R12 [R9 + PIPE_COUNT]

    ; copied++ loop counter
0x00003710       ADD R10 R10 1
0x00003714       B pipe_read_loop

pipe_read_done:
; wake blocked writers
0x0000371C       MOV R1 R9
0x00003720       ADD R1 R1 PIPE_WWAIT
0x00003724       BL waitq_wake_all
0x0000372C       MOV R1 R10          ; read bytes amount
0x00003730       POP LR
0x00003734       RET

pipe_read_badptr:
0x00003738       LI R1 ERR_FAULT
0x00003740       POP LR
0x00003744       RET
...
; prepare sleep
;------------------------------------------
0x00003748       MOV R1 R9
0x0000374C       ADD R1 R1 PIPE_RWAIT    ;ptr on wait queue read in pipe instance
0x00003750       LI R2 WAIT_PIPE_READ    ;REASON for block in process (debug)
0x00003758       BL waitq_prepare_sleep

;------------------------------------------
; race check
;------------------------------------------
0x00003760       LDW R4 [R9 + PIPE_COUNT]
0x00003764       CMP R4 0
0x00003768       BNE pipe_read_retry

0x00003770       BL waitq_sleep_current  ;freesze here untill unblock
    ;data arrived/unbloked
0x00003778       B pipe_read_retry
...
pipe_loop:
0x00003788       LI  R1 MAX_PIPES
0x00003790       CMP R2 R1
0x00003794       BGE pipe_alloc_fail

0x0000379C       SHL R3 R2 2

0x000037A0       LI R4 pipe_used
0x000037A8       ADD R4 R4 R3

0x000037AC       LDW R5 [R4]             ;R4 address in PIPE_USED LIST

0x000037B0       CMP R5 0                ; 0 -empty
0x000037B4       BEQ pipe_found

0x000037BC       ADD R2 R2 1
0x000037C0       B pipe_loop

pipe_found:

0x000037C8       LI R5 1
0x000037D0       STW R5 [R4]             ; set it in PIPE_USED =1 as used

0x000037D4       LI R4 PIPE_SIZE
0x000037DC       MUL R6 R2 R4            ; r2 - is idx so get full offset = PIPE_SIZE*idx

0x000037E0       LI R1 pipe_pool         ; R1 - is address of the to be allocated slot in pipe_pool
0x000037E8       ADD R1 R1 R6

0x000037EC       LI R7 0                 ; clean it up
0x000037F4       STW R7 [R1 + PIPE_HEAD]
0x000037F8       STW R7 [R1 + PIPE_TAIL]
0x000037FC       STW R7 [R1 + PIPE_COUNT]
0x00003800       STW R7 [R1 + PIPE_RWAIT]
0x00003804       STW R7 [R1 + PIPE_WWAIT]
...
0x00003818       LI R2 pipe_pool
0x00003820       SUB R3 R1 R2

0x00003824       LI R4 PIPE_SIZE
0x0000382C       DIV R5 R3 R4

0x00003830       SHL R5 R5 2
0x00003834       LI R6 pipe_used
0x0000383C       ADD R6 R6 R5

0x00003840       LI R7 0
0x00003848       STW R7 [R6]
...
0x00003854       MOV R9 R1
0x00003858       MOV R7 R2
0x0000385C       MOV R6 R3

0x00003860       LDW R9 [R9 + FILE_INODE]
0x00003864       LDW R9 [R9 + INODE_PRIVATE] ;get our Pipe instance allocated in pipe_pool (pipe*) (from its inode)
...
0x00003870       MOV R1 R7
0x00003874       MOV R2 R6
0x00003878       LI  R3 0           ; READ access
0x00003880       BL user_buffer_valid_range

0x00003888       POP R6
0x0000388C       POP R7

0x00003890       CMP R1 1
0x00003894       BNE pipe_write_badptr

0x0000389C       LI R10 0               ; bytes written
pipe_write_retry:
0x000038A4       CMP R10 R6
0x000038A8       BGE pipe_write_done
;------------------------------------------
; pipe full ?
;------------------------------------------
0x000038B0       LDW R11 [R9 + PIPE_COUNT]
0x000038B4       LI R2 256
0x000038BC       CMP R11 R2
0x000038C0       BEQ pipe_write_sleep
;------------------------------------------
; head = pipe->head
;------------------------------------------
0x000038C8       LDW R12 [R9 + PIPE_HEAD]

0x000038CC       MOV R4 R7
0x000038D0       ADD R4 R4 R10
0x000038D4       LDB R5 [R4]     ; read byte from user buff addr

0x000038D8       MOV R4 R9
0x000038DC       ADD R4 R4 PIPE_BUFFER
0x000038E0       ADD R4 R4 R12
0x000038E4       STB R5 [R4]     ; put it to pipe addr - ie write user -> pipe buff

;------------------------------------------
; head=(head+1)&255
;------------------------------------------
0x000038E8       ADD R12 R12 1
0x000038EC       LI R2 255
0x000038F4       AND R12 R12 R2
0x000038F8       STW R12 [R9 + PIPE_HEAD]
;------------------------------------------
; count++
;------------------------------------------
0x000038FC       LDW R4 [R9 + PIPE_COUNT]
0x00003900       ADD R4 R4 1
0x00003904       STW R4 [R9 + PIPE_COUNT]

; written++
0x00003908       ADD R10 R10 1
0x0000390C       B pipe_write_retry

pipe_write_done:
; wake readers
0x00003914       MOV R1 R9
0x00003918       ADD R1 R1 PIPE_RWAIT    ; wq ptr from pipe*
0x0000391C       BL waitq_wake_all
0x00003924       MOV R1 R10      ;written bytes
0x00003928       POP LR
0x0000392C       RET

pipe_write_badptr:
0x00003930       LI R1 ERR_FAULT
0x00003938       POP LR
0x0000393C       RET
...
0x00003950       MOV R1 R9
0x00003954       ADD R1 R1 PIPE_WWAIT    ; wq ptr from pipe*
0x00003958       LI R2 WAIT_PIPE_WRITE
0x00003960       BL waitq_prepare_sleep
    ; race check
0x00003968       LDW R4 [R9 + PIPE_COUNT]
0x0000396C       LI R2 256
0x00003974       CMP R4 R2
0x00003978       BLT pipe_write_retry    ;if not full dont block/frezze go write

0x00003980       BL waitq_sleep_current  ;block anf freeze writer here until reading buffer frees room in pipe!

0x00003988       B pipe_write_retry      ; unblocked! go write!
...
0x00003990       CMP R1 3
0x00003994       BLT fd_lookup_invalid       ; fd 0,1,2 - stdio, нельзя закрыть пользователю
0x0000399C       CMP R1 MAX_FDS
0x000039A0       BGE fd_lookup_invalid       ; fd >= MAX_FDS - вне диапазона

0x000039A8       MOV R8 R1                   ; сохраняем fd
    ; Получаем указатель на fd_table текущего процесса
; macro: GET_CURR_TASK_IDX R4
0x000039AC   LI R1 CURRENT_TASK
0x000039B4   LDW R4 [R1]
; macro: GET_TASK_PTR R4, R4
0x000039B8   LI R1 TASK_SIZE
0x000039C0   MUL R3 R4 R1
0x000039C4   LI R4 tasks
0x000039CC   ADD R4 R4 R3
; macro: TASK_GET_FD_TABLE R4, R4    ; R4 = &fd_table[0]
0x000039D0   LDW R4 [R4 + TASK_FD_TABLE]

    ; Вычисляем адрес fd_table[fd]
0x000039D4       SHL R5 R8 2                 ; R5 = fd * 4 (размер указателя)
0x000039D8       ADD R6 R4 R5                ; R6 = &fd_table[fd]

0x000039DC       LDW R1 [R6]                 ; R1 = file* из таблицы
0x000039E0       CMP R1 0
0x000039E4       BEQ fd_lookup_invalid       ; если NULL - дескриптор не занят

0x000039EC       MOV R2 R6                   ; возвращаем адрес ячейки для fd_remove
0x000039F0       RET
...
0x00003A04       RET
 ...
 fd_remove:
0x00003A08       PUSH LR
0x00003A0C       BL  fd_lookup
0x00003A14       CMP R1 0
0x00003A18       BEQ fd_remove_invalid

0x00003A20       MOV R8 R1          ; сохраняем file*
0x00003A24       LI R3 0
0x00003A2C       STW R3 [R2]        ; fd_table[fd] = NULL (R2 из fd_lookup)
0x00003A30       MOV R1 R8          ; file*
0x00003A34       POP LR
0x00003A38       RET
...
0x00003A4C       LDW R1 [SP + TF_R1]
0x00003A50       LDW R2 [SP + TF_R2]
0x00003A54       LDW R3 [SP + TF_R3]

0x00003A58       BL vfs_read

0x00003A60       STW R1 [SP + TF_R1]
...
0x00003A94       PUSH R11
0x00003A98       PUSH R12
0x00003A9C       MOV R9 R1
0x00003AA0       MOV R7 R2
0x00003AA4       MOV R6 R3
0x00003AA8       LI R8 0                    ; total bytes collected
0x00003AB0       LDW R9 [R9 + FILE_INODE]
0x00003AB4       LDW R9 [R9 + INODE_PRIVATE] ; console device pointer
0x00003AB8       CMP R6 0
0x00003ABC       BEQ read_done

0x00003AC4       PUSH R7
0x00003AC8       PUSH R6
0x00003ACC       PUSH R9
0x00003AD0       MOV R1 R7
0x00003AD4       MOV R2 R6
0x00003AD8       LI R3 1                ; write access for destination buffer
0x00003AE0       BL user_buffer_valid_range
0x00003AE8       POP R9
0x00003AEC       POP R6
0x00003AF0       POP R7
0x00003AF4       CMP R1 1
0x00003AF8       BNE con_read_fault

read_wait_uart_rx:
0x00003B00       LDW R4 [R9 + UARTDEV_MMIO]  ; UART MMIO Base Address
0x00003B04       LDW R5 [R4 + 4]             ; read UART_STATUS register
0x00003B08       AND R5 R5 1                 ; bit 0 = RX_READY
0x00003B0C       CMP R5 0
0x00003B10       BEQ read_block_uart_rx      ; bit 0=0 no data yet in rx_queue, block this curr user task inside syscall

; macro: GET_CURR_TASK_IDX R4
0x00003B18   LI R1 CURRENT_TASK
0x00003B20   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00003B24   LI R1 TASK_SIZE
0x00003B2C   MUL R3 R4 R1
0x00003B30   LI R5 tasks
0x00003B38   ADD R5 R5 R3
; macro: TASK_GET_KBUF_RD R1, R5
0x00003B3C   LDW R1 [R5 + TASK_KBUF_RD_PTR]
0x00003B40       MOV R2 R6
0x00003B44       MOV R3 R9
0x00003B48       PUSH R6
0x00003B4C       PUSH R7
0x00003B50       PUSH R8
0x00003B54       PUSH R9
0x00003B58       BL device_read          ;read data from rx_queue to KBUFFER_RD len=R2(<- R6) or if 0xd (enter sign)
0x00003B60       POP R9
0x00003B64       POP R8
0x00003B68       POP R7
0x00003B6C       POP R6
...
0x00003B70       CMP R1 0
0x00003B74       BEQ read_wait_uart_rx

0x00003B7C       MOV R10 R1             ; actual bytes read

; macro: GET_CURR_TASK_IDX R5
0x00003B80   LI R1 CURRENT_TASK
0x00003B88   LDW R5 [R1]
; macro: GET_TASK_PTR R4, R5
0x00003B8C   LI R1 TASK_SIZE
0x00003B94   MUL R3 R5 R1
0x00003B98   LI R4 tasks
0x00003BA0   ADD R4 R4 R3
; macro: TASK_GET_KBUF_RD R4, R4
0x00003BA4   LDW R4 [R4 + TASK_KBUF_RD_PTR]

    ; Remember whether this chunk ended with newline before copy_to_user
    ; clobbers temporary registers.
0x00003BA8       LI R11 0
0x00003BB0       SUB R5 R10 1
0x00003BB4       ADD R5 R4 R5
0x00003BB8       LDB R5 [R5]
0x00003BBC       CMP R5 10
0x00003BC0       BNE read_chunk_not_newline
0x00003BC8       LI R11 1
...
0x00003BE0       PUSH R10
0x00003BE4       PUSH R11
0x00003BE8       MOV R1 R7              ; user destination
0x00003BEC       MOV R2 R10
0x00003BF0       BL copy_to_user        ; copy from kernel buffer to user buffer
0x00003BF8       POP R11
0x00003BFC       POP R10
0x00003C00       POP R9
0x00003C04       POP R8
...
0x00003C10       ADD R7 R7 R10
0x00003C14       ADD R8 R8 R10
0x00003C18       SUB R6 R6 R10

0x00003C1C       CMP R11 1
0x00003C20       BEQ read_complete
0x00003C28       CMP R6 0
0x00003C2C       BGT read_wait_uart_rx

read_complete:
0x00003C34       MOV R1 R8
0x00003C38       B read_return
...
0x00003C40       LI R1 uart_rx_waitq
0x00003C48       LI R2 WAIT_UART_RX
0x00003C50       BL waitq_prepare_sleep

0x00003C58       LDW R4 [R9 + UARTDEV_MMIO]
0x00003C5C       LDW R10 [R4 + 4]             ; re-check uart reg RX-ready bit 0 after marking blocked
0x00003C60       AND R10 R10 1
0x00003C64       CMP R10 0
0x00003C68       BNE read_unblock_uart_rx     ; if data arrived, cancel sleep and read it

0x00003C70       BL waitq_sleep_current       ; save this user task as frozen in kernel space

0x00003C78       B read_wait_uart_rx          ;repeat read uart loop

read_unblock_uart_rx:            ;mark current task as unblocked
0x00003C80       LI R1 uart_rx_waitq
0x00003C88       BL waitq_cancel_sleep_current

0x00003C90       B read_wait_uart_rx          ;go back and read bytes

read_done:
0x00003C98       LI R1 0
0x00003CA0       B read_return

con_read_fault:
0x00003CA8       LI R1 ERR_FAULT
...
0x00003CCC       LDW R1 [SP + TF_R1]
0x00003CD0       LDW R2 [SP + TF_R2]
0x00003CD4       LDW R3 [SP + TF_R3]

0x00003CD8       BL vfs_write

0x00003CE0       STW R1 [SP + TF_R1]
0x00003CE4       B trap_restore
...
0x00003CEC       PUSH LR
0x00003CF0       MOV R9 R1
0x00003CF4       MOV R7 R2
0x00003CF8       MOV R6 R3
0x00003CFC       LDW R9 [R9 + FILE_INODE]
0x00003D00       LDW R9 [R9 + INODE_PRIVATE] ; console device pointer
0x00003D04       LI R8 0                    ; total bytes written
                               ;also R6-len R7-user buf ptr R9-file struc ptr
write_loop:
0x00003D0C       CMP R6 0
0x00003D10       BEQ write_done             ;0 bytes

0x00003D18       LI R2 KBUFFER_SIZE
0x00003D20       CMP R6 R2                  ;here we write in chunks to dev, last one is small chunk (less then Kbuffer_size)
0x00003D24       BLT write_chunk_small
0x00003D2C       LI R2 KBUFFER_SIZE

0x00003D34       B write_chunk

write_chunk_small:
0x00003D3C       MOV R2 R6
...
0x00003D48       PUSH R9
0x00003D4C       PUSH R8
0x00003D50       MOV R1 R7
0x00003D54       MOV R2 R2
0x00003D58       LI R3 0                ; read access for source buffer
0x00003D60       BL user_buffer_valid_range ;Validate user buffer and length for this chunk
0x00003D68       POP R8
0x00003D6C       POP R9
0x00003D70       POP R6
0x00003D74       POP R7
0x00003D78       CMP R1 1
0x00003D7C       BNE driver_bad_pointer
...
0x00003D88       PUSH R6
    ...
    ;=================================================
; macro: GET_CURR_TASK_IDX R4
0x00003D8C   LI R1 CURRENT_TASK
0x00003D94   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00003D98   LI R1 TASK_SIZE
0x00003DA0   MUL R3 R4 R1
0x00003DA4   LI R5 tasks
0x00003DAC   ADD R5 R5 R3
; macro: TASK_GET_KBUF_WR R4, R5
0x00003DB0   LDW R4 [R5 + TASK_KBUF_WR_PTR]
0x00003DB4       MOV R1 R7
0x00003DB8       BL copy_from_user      ; copy chunk to tasks kbuffer_wr
0x00003DC0       MOV R10 R1             ; bytes copied
0x00003DC4       POP R6
0x00003DC8       POP R7
...
write_wait_uart_tx:
0x00003DD8       LDW R1 [R9 + UARTDEV_MMIO]
0x00003DDC       LDW R2 [R1 + 4]
0x00003DE0       AND R2 R2 2                     ;check bit 1 - UART_TX rdy
0x00003DE4       CMP R2 0
0x00003DE8       BEQ write_block_uart_tx         ;not rdy go and block this task

; can TX to UART!

; macro: GET_CURR_TASK_IDX R4
0x00003DF0   LI R1 CURRENT_TASK
0x00003DF8   LDW R4 [R1]
; macro: GET_TASK_PTR R5, R4
0x00003DFC   LI R1 TASK_SIZE
0x00003E04   MUL R3 R4 R1
0x00003E08   LI R5 tasks
0x00003E10   ADD R5 R5 R3
; macro: TASK_GET_KBUF_WR R1, R5
0x00003E14   LDW R1 [R5 + TASK_KBUF_WR_PTR]
0x00003E18       MOV R2 R10
0x00003E1C       MOV R3 R9
    ;============================================================================
...
0x00003E20       BL device_write
0x00003E28       POP R6
0x00003E2C       POP R9
0x00003E30       POP R7

0x00003E34       CMP R1 0        ;nothing is written - go again
0x00003E38       BEQ write_loop

0x00003E40       ADD R8 R8 R1     ;update ptrs
0x00003E44       ADD R7 R7 R1     ;R7 pointer in user buffer R8-who knows?
0x00003E48       SUB R6 R6 R1     ;decrease amounts for next chunk to send
0x00003E4C       B write_loop     ;chunk is sent go to next one
...
0x00003E54       LI R1 uart_tx_waitq
0x00003E5C       LI R2 WAIT_UART_TX
0x00003E64       BL waitq_prepare_sleep

0x00003E6C       LDW R1 [R9 + UARTDEV_MMIO]
0x00003E70       LDW R2 [R1 + 4]             ; re-check after marking blocked
0x00003E74       AND R2 R2 2
0x00003E78       CMP R2 0
0x00003E7C       BNE write_unblock_uart_tx   ; if suddenly TX ready - unblock it
...
0x00003E84       BL waitq_sleep_current      ; if task is blocked it sleeps here inside syscall line waiting for irq UART handler ublocks it
                                ; (when TX rdy)
                                ; also this call saves task in trapframe and jumps to schedule and switch other tasks
0x00003E8C       B write_wait_uart_tx        ; task awakes here - jumps send uart again!!

write_unblock_uart_tx:
0x00003E94       LI R1 uart_tx_waitq
0x00003E9C       BL waitq_cancel_sleep_current

0x00003EA4       B write_wait_uart_tx

write_done:
0x00003EAC       MOV R1 R8
0x00003EB0       POP LR
0x00003EB4       RET

driver_bad_pointer:
0x00003EB8       LI R1 ERR_FAULT
0x00003EC0       POP LR
0x00003EC4       RET

bad_fd:
0x00003EC8       LI R1 ERR_BADF
0x00003ED0       STW R1 [SP + TF_R1]

0x00003ED4       B trap_restore

bad_pointer:
0x00003EDC       LI R1 ERR_FAULT
0x00003EE4       STW R1 [SP + TF_R1]

0x00003EE8       B trap_restore

file_read:
    ;================================================================
    ; R1 = file ptr, R2 = user buffer, R3 = len
    ;================================================================
0x00003EF0       LDW R4 [R1 + FILE_INODE]
0x00003EF4       LDW R4 [R4 + INODE_OPS]
0x00003EF8       LDW R4 [R4 + FSOPS_READ]
0x00003EFC       JR R4
...
0x00003F08       LDW R4 [R4 + FSOPS_WRITE]    ; get write function xdev_write from ops
0x00003F0C       JR R4                       ; execute it
...
0x00003F18       B uart_write_kernel
...
0x00003F20       LDW R4 [R3 + UARTDEV_MMIO]  ; UART MMIO Base Address
0x00003F24       LI R5 0                     ; index = 0 (bytes read so far)

dr_loop:
0x00003F2C       CMP R5 R2                   ; have we read enough bytes?
0x00003F30       BGE dr_done                 ; yes -> return

dr_poll_ready:
0x00003F38       LDW R6 [R4 + 4]             ; read UART_STATUS register
0x00003F3C       AND R6 R6 1                 ; bit 0 = RX_READY
0x00003F40       CMP R6 0
0x00003F44       BEQ dr_done                 ; no more buffered input available

0x00003F4C       LDW R7 [R4 + 0]             ; pop character from UART_DATA (RX FIFO)
0x00003F50       STB R7 [R1 + R5]            ; store it into the kernel buffer
0x00003F54       ADD R5 R5 1

    ; If we received a newline, stop reading early
0x00003F58       CMP R7 10
0x00003F5C       BEQ dr_done

0x00003F64       B dr_loop

dr_done:
0x00003F6C       MOV R1 R5                   ; return number of bytes actually read
0x00003F70       RET
...
0x00003F74       LDW R4 [R3 + UARTDEV_MMIO]  ; UART MMIO Base Address
0x00003F78       LI R5 0                     ; index = 0 (bytes written so far)

dcw_loop:
0x00003F80       CMP R5 R2                   ; have we written all bytes?
0x00003F84       BGE dcw_done                ; yes -> return

dcw_poll_tx:
0x00003F8C       LDW R6 [R4 + 4]             ; read UART_STATUS register
0x00003F90       AND R6 R6 2                 ; bit 1 = TX_READY
0x00003F94       CMP R6 0
0x00003F98       BEQ dcw_done

0x00003FA0       LDB R7 [R1 + R5]            ; load next byte from kernel buffer
0x00003FA4       STW R7 [R4 + 0]             ; write to UART_DATA register (transmit)
0x00003FA8       ADD R5 R5 1
0x00003FAC       B dcw_loop

dcw_done:
0x00003FB4       MOV R1 R5                   ; return number of bytes written
0x00003FB8       RET
...
0x00003FC8       PUSH LR
0x00003FCC       MOV R6 R3
0x00003FD0       CMP R6 0
0x00003FD4       BEQ null_write_done

0x00003FDC       PUSH R6
0x00003FE0       MOV R1 R2
0x00003FE4       MOV R2 R6
0x00003FE8       LI R3 0                    ; read access from user source
0x00003FF0       BL user_buffer_valid_range
0x00003FF8       POP R6
0x00003FFC       CMP R1 1
0x00004000       BNE null_write_badptr

null_write_done:
0x00004008       MOV R1 R6
0x0000400C       POP LR
0x00004010       RET

null_write_badptr:
0x00004014       LI R1 ERR_FAULT
0x0000401C       POP LR
0x00004020       RET
...
0x00004030       CMP R1 0
0x00004034       BLT fd_invalid
0x0000403C       CMP R1 MAX_FDS
0x00004040       BGE fd_invalid

0x00004048       MOV R8 R1                   ; preserve fd across task lookup macros
; macro: GET_CURR_TASK_IDX R4
0x0000404C   LI R1 CURRENT_TASK
0x00004054   LDW R4 [R1]
; macro: GET_TASK_PTR R4, R4
0x00004058   LI R1 TASK_SIZE
0x00004060   MUL R3 R4 R1
0x00004064   LI R4 tasks
0x0000406C   ADD R4 R4 R3
; macro: TASK_GET_FD_TABLE R4, R4
0x00004070   LDW R4 [R4 + TASK_FD_TABLE]

0x00004074       SHL R5 R8 2
0x00004078       ADD R4 R4 R5                ; r4=fd*4+FD_TABLE
0x0000407C       LDW R1 [R4]                 ; R1 = file ptr
0x00004080       LDW R6 [R1 + FILE_FLAGS]
0x00004084       AND R6 R6 R2
0x00004088       CMP R6 R2
0x0000408C       BNE fd_invalid

0x00004094       POP R8
0x00004098       POP R6
0x0000409C       POP R5
0x000040A0       RET                         ;on exit R1 - has file ptr
...
0x000040BC       PUSH LR
0x000040C0       MOV R7 R2
0x000040C4       MOV R10 R3

0x000040C8       LI R2 FD_FLAG_READ
0x000040D0       BL fetch_fd_entry   ; macro inside destroys R6

0x000040D8       CMP R1 0
0x000040DC       BEQ vfs_read_badfd

0x000040E4       MOV R9 R1
0x000040E8       MOV R1 R9
0x000040EC       MOV R2 R7
0x000040F0       MOV R3 R10
0x000040F4       BL file_read
0x000040FC       POP LR
0x00004100       RET

vfs_read_badfd:
0x00004104       LI R1 ERR_BADF
0x0000410C       POP LR
0x00004110       RET
...
0x00004114       PUSH LR
0x00004118       MOV R7 R2
0x0000411C       MOV R10 R3

0x00004120       LI R2 FD_FLAG_WRITE
0x00004128       BL fetch_fd_entry   ;macro inside desroys R6 (fixed)

0x00004130       CMP R1 0
0x00004134       BEQ vfs_write_badfd

0x0000413C       MOV R9 R1
0x00004140       MOV R1 R9           ; R1 - file* acc to fd
0x00004144       MOV R2 R7
0x00004148       MOV R3 R10
0x0000414C       BL file_write
0x00004154       POP LR
0x00004158       RET

vfs_write_badfd:
0x0000415C       LI R1 ERR_BADF
0x00004164       POP LR
0x00004168       RET
...
0x00004178       LI R4 0
0x00004180       CMP R2 R4
0x00004184       BEQ uv_valid

0x0000418C       LI R4 USER_BASE
0x00004194       CMP R1 R4
0x00004198       BLT uv_invalid

0x000041A0       LI R4 USER_LIMIT
0x000041A8       ADD R5 R1 R2
0x000041AC       SUB R5 R5 1
0x000041B0       CMP R5 R1
0x000041B4       BLT uv_invalid
0x000041BC       CMP R5 R4
0x000041C0       BGT uv_invalid
0x000041C8       MOV R11 R1              ; save start address; task macros clobber R1
0x000041CC       MOV R12 R5              ; save end address for page calculation
0x000041D0       MOV R4 R3               ; save access type; task macros clobber R3

; macro: GET_CURR_TASK_IDX R6
0x000041D4   LI R1 CURRENT_TASK
0x000041DC   LDW R6 [R1]
; macro: GET_TASK_PTR R6, R6
0x000041E0   LI R1 TASK_SIZE
0x000041E8   MUL R3 R6 R1
0x000041EC   LI R6 tasks
0x000041F4   ADD R6 R6 R3
; macro: TASK_GET_PTBR R6, R6
0x000041F8   LDW R6 [R6 + TASK_PTBR]
    ; Dynamic page tables live in the supervisor-only allocator pool,
    ; which is identity-mapped into every task address space.
0x000041FC       CMP R6 0
0x00004200       BEQ uv_invalid

uv_check_pages:
0x00004208       SHR R7 R11 12
0x0000420C       SHR R8 R12 12
uv_loop:
    ;================================================================
...
0x00004210       CMP R7 R8
0x00004214       BGT uv_valid
0x0000421C       SHL R9 R7 2
0x00004220       ADD R9 R9 R6
0x00004224       LDW R10 [R9]
0x00004228       AND R5 R10 PTE_P
0x0000422C       CMP R5 0
0x00004230       BEQ uv_invalid
0x00004238       AND R5 R10 PTE_U
0x0000423C       CMP R5 0
0x00004240       BEQ uv_invalid
0x00004248       CMP R4 0
0x0000424C       BEQ uv_check_read
0x00004254       AND R5 R10 PTE_W
0x00004258       CMP R5 0
0x0000425C       BEQ uv_invalid
0x00004264       B uv_next

uv_check_read:
0x0000426C       AND R5 R10 PTE_R
0x00004270       CMP R5 0
0x00004274       BEQ uv_invalid

uv_next:
0x0000427C       ADD R7 R7 1
0x00004280       B uv_loop
...
cfu_head:
0x000042C0       CMP R2 0
0x000042C4       BEQ cfu_done
0x000042CC       OR R6 R1 R4
0x000042D0       AND R6 R6 3
0x000042D4       CMP R6 0
0x000042D8       BEQ cfu_word
0x000042E0       LDB R7 [R1]
0x000042E4       STB R7 [R4]
0x000042E8       ADD R1 R1 1
0x000042EC       ADD R4 R4 1
0x000042F0       ADD R5 R5 1
0x000042F4       SUB R2 R2 1
0x000042F8       B cfu_head
cfu_word:
0x00004300       CMP R2 4
0x00004304       BLT cfu_tail
0x0000430C       LDW R7 [R1]
0x00004310       STW R7 [R4]
0x00004314       ADD R1 R1 4
0x00004318       ADD R4 R4 4
0x0000431C       ADD R5 R5 4
0x00004320       SUB R2 R2 4
0x00004324       B cfu_word
cfu_tail:
0x0000432C       CMP R2 0
0x00004330       BEQ cfu_done
0x00004338       LDB R7 [R1]
0x0000433C       STB R7 [R4]
0x00004340       ADD R1 R1 1
0x00004344       ADD R4 R4 1
0x00004348       ADD R5 R5 1
0x0000434C       SUB R2 R2 1
0x00004350       B cfu_tail
cfu_done:
0x00004358       MOV R1 R5
0x0000435C       RET
...
ctu_head:
0x00004368       CMP R2 0
0x0000436C       BEQ ctu_done
0x00004374       OR R6 R1 R4
0x00004378       AND R6 R6 3
0x0000437C       CMP R6 0
0x00004380       BEQ ctu_word
0x00004388       LDB R7 [R4]
0x0000438C       STB R7 [R1]
0x00004390       ADD R1 R1 1
0x00004394       ADD R4 R4 1
0x00004398       ADD R5 R5 1
0x0000439C       SUB R2 R2 1
0x000043A0       B ctu_head
ctu_word:
0x000043A8       CMP R2 4
0x000043AC       BLT ctu_tail
0x000043B4       LDW R7 [R4]
0x000043B8       STW R7 [R1]
0x000043BC       ADD R1 R1 4
0x000043C0       ADD R4 R4 4
0x000043C4       ADD R5 R5 4
0x000043C8       SUB R2 R2 4
0x000043CC       B ctu_word
ctu_tail:
0x000043D4       CMP R2 0
0x000043D8       BEQ ctu_done
0x000043E0       LDB R7 [R4]
0x000043E4       STB R7 [R1]
0x000043E8       ADD R1 R1 1
0x000043EC       ADD R4 R4 1
0x000043F0       ADD R5 R5 1
0x000043F4       SUB R2 R2 1
0x000043F8       B ctu_tail
ctu_done:
0x00004400       MOV R1 R5
0x00004404       RET

handle_debug:
    ; Debug trap - just return
0x00004408       B trap_restore
...
0x00004410       CSRR R1 STVAL

0x00004414       CMP R1 0
0x00004418       BEQ handle_timer_irq

0x00004420       CMP R1 1
0x00004424       BEQ handle_uart_irq
    ;================================================================
    ; Default IRQ handling: acknowledge PIC and restore
    ;================================================================
0x0000442C       LI R2 0x00102000
0x00004434       STW R1 [R2 + 8]             ; PIC_ACK = R1
0x00004438       B trap_restore
...
0x00004440       LI R2 0x00102000
0x00004448       LI R3 0
0x00004450       STW R3 [R2 + 8]             ; PIC_ACK = 0

    ; Increment timer tick counter
0x00004454       LI R1 timer_ticks
0x0000445C       LDW R2 [R1]
0x00004460       ADD R2 R2 1
0x00004464       STW R2 [R1]

    ;================================================================
    ; Wake sleeping tasks whose time has expired
    ;================================================================

0x00004468       LI R1 sleep_waitq
0x00004470       LDW R8 [R1]                ; R8 = current sleep_waitq mask
0x00004474       LI R9 0                    ; R9 = tasks to wake bitmask
0x0000447C       LI R3 0                    ; task index

timer_wake_scan:
0x00004484       CMP R3 MAX_TASKS
0x00004488       BGE timer_wake_scan_done

    ; Check if this task is in the sleep wait queue
0x00004490       LI R6 1
0x00004498       SHL R6 R6 R3               ; bit for this task
0x0000449C       AND R7 R8 R6
0x000044A0       CMP R7 0
0x000044A4       BEQ timer_wake_next        ; not in sleep queue

    ; Task is sleeping, check if it's time to wake
; macro: GET_TASK_PTR R5, R3
0x000044AC   LI R1 TASK_SIZE
0x000044B4   MUL R3 R3 R1
0x000044B8   LI R5 tasks
0x000044C0   ADD R5 R5 R3
; macro: TASK_GET_WAKE_TIME R7, R5
0x000044C4   LDW R7 [R5 + TASK_WAKE_TIME]
0x000044C8       CMP R2 R7                  ; current time >= wake time?
0x000044CC       BLT timer_wake_next

    ; Mark this task for wakeup
0x000044D4       OR R9 R9 R6                 ; add to wake bitmask bitwize

timer_wake_next:
0x000044D8       ADD R3 R3 1
0x000044DC       B timer_wake_scan

timer_wake_scan_done:
    ; If no tasks to wake, skip
0x000044E4       CMP R9 0
0x000044E8       BEQ timer_no_wake

    ; Wake the expired tasks using our new function
0x000044F0       LI R1 sleep_waitq
0x000044F8       MOV R2 R9
0x000044FC       BL waitq_wake_bitmask

timer_no_wake:

    ; Yield the CPU (reschedule and switch tasks)
0x00004504       B schedule_and_switch
...
0x0000450C       LI R2 0x00102000
0x00004514       LI R3 1
0x0000451C       STW R3 [R2 + 8]             ; PIC_ACK = 1
...
0x00004520       LI R1 uart_rx_waitq
0x00004528       BL waitq_wake_all
0x00004530       LI R1 uart_tx_waitq
0x00004538       BL waitq_wake_all

uart_wake_done:
    ; Resume the interrupted task immediately
0x00004540       B trap_restore
...
0x00004548       POP R1                  ; stval, informational only
0x0000454C       POP R1                  ; scause, informational only
0x00004550       POP R1
0x00004554       CSRW SSTATUS R1
0x00004558       POP R1
0x0000455C       CSRW SFLAGS R1
0x00004560       POP R1
0x00004564       CSRW SEPC R1
0x00004568       POP R1                  ; interrupted task SP
0x0000456C       CSRW SSCRATCH R1        ; task SP goes to SSCRATCH
...
0x000045A8       CSRRW SP SSCRATCH SP
0x000045AC       SRET
