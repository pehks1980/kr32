...
0x000440CE       LI  R3 256
0x000440D6       SUB SP SP R3
0x000440DA       MOV R12 SP              ; R12 = buffer pointer

0x000440DE       MOV R8 R1               ; R8 = argc
0x000440E2       MOV R9 R2               ; R9 = argv

0x000440E6       CMP R8 2                ; Need at least one argument (argv[1])
0x000440EA       BLT usage

0x000440F2       LI R10 1                ; R10 = current argument index (argv[1])
0x000440FA       LI R6 0                 ; R6 = return code (0 = success)

dir_loop:
0x00044102       CMP R10 R8              ; if index >= argc, done
0x00044106       BGE dir_done

    ; Get the path string from argv[index]
0x0004410E       MOV R2 R10
0x00044112       SHL R2 R2 2
0x00044116       ADD R2 R9 R2
0x0004411A       LDW R1 [R2]             ; R1 = directory path (e.g., "etc/")
0x0004411E       PUSH R1

    ; Print header: "\n--- Directory: path ---\n"
0x00044122       LI R1 newline_str
0x0004412A       BL puts
0x00044132       LI R1 dir_header_prefix
0x0004413A       BL puts
    ; print the directory name
0x00044142       MOV R2 R10
0x00044146       SHL R2 R2 2
0x0004414A       ADD R2 R9 R2
0x0004414E       LDW R1 [R2]
0x00044152       BL puts
0x0004415A       LI R1 dir_header_suffix
0x00044162       BL puts
0x0004416A       LI R1 newline_str
0x00044172       BL puts

    ; open directory
    ; R1 already has the path
0x0004417A       POP R1
0x0004417E       LI  R2 O_RDONLY
0x00044186       BL open
0x0004418E       MOV R11 R1              ; R11 = fd

0x00044192       CMP R11 0
0x00044196       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x0004419E       MOV R1 R11
0x000441A2       MOV R2 R12
0x000441A6       LI  R3 DIRENT_SIZEOF
0x000441AE       BL read
0x000441B6       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x000441BA       CMP R7 0
0x000441BE       BEQ read_done           ; EOF
0x000441C6       CMP R7 DIRENT_SIZEOF
0x000441CA       BNE read_done           ; error or short read

    ; parse the directory entry
0x000441D2       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x000441D6       ADD R1 R12 DIRENT_NAME
0x000441DA       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x000441E2       CMP R5 DT_DIR
0x000441E6       BNE not_dir_entry
0x000441EE       LI R1 slash_str
0x000441F6       BL puts
not_dir_entry:

    ; print newline
0x000441FE       LI R1 newline_str
0x00044206       BL puts

0x0004420E       B read_dir_loop

read_done:
    ; close(fd)
0x00044216       MOV R1 R11
0x0004421A       BL close

0x00044222       ADD R10 R10 1           ; next directory
0x00044226       B dir_loop

open_failed:
    ; print error message for this directory
0x0004422E       LI R1 error_prefix
0x00044236       BL puts
    ; print the directory name
0x0004423E       MOV R2 R10
0x00044242       SHL R2 R2 2
0x00044246       ADD R2 R9 R2
0x0004424A       LDW R1 [R2]
0x0004424E       BL puts
0x00044256       LI R1 newline_str
0x0004425E       BL puts

0x00044266       LI R6 1                 ; set return code to error
0x0004426E       ADD R10 R10 1           ; next directory
0x00044272       B dir_loop

dir_done:
    ; free buffer
0x0004427A       LI  R2 256
0x00044282       ADD SP SP R2
0x00044286       MOV R1 R6               ; return code
0x0004428A       POP R12
0x0004428E       POP R11
0x00044292       POP R10
0x00044296       POP R9
...
;==============================================================================
usage:
0x000442AE       LI R1 usage_str
0x000442B6       BL puts
0x000442BE       LI R6 1                 ; error
0x000442C6       B dir_done
