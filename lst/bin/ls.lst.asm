...
0x00043BFA       LI  R3 256
0x00043C02       SUB SP SP R3
0x00043C06       MOV R12 SP              ; R12 = buffer pointer
...
0x00043CA6       POP R1
0x00043CAA       LI  R2 O_RDONLY
0x00043CB2       BL open
0x00043CBA       MOV R11 R1              ; R11 = fd

0x00043CBE       CMP R11 0
0x00043CC2       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x00043CCA       MOV R1 R11
0x00043CCE       MOV R2 R12
0x00043CD2       LI  R3 DIRENT_SIZEOF
0x00043CDA       BL read
0x00043CE2       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043CE6       CMP R7 0
0x00043CEA       BEQ read_done           ; EOF
0x00043CF2       CMP R7 DIRENT_SIZEOF
0x00043CF6       BNE read_done           ; error or short read

    ; parse the directory entry
0x00043CFE       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00043D02       ADD R1 R12 DIRENT_NAME
0x00043D06       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x00043D0E       CMP R5 DT_DIR
0x00043D12       BNE not_dir_entry
0x00043D1A       LI R1 slash_str
0x00043D22       BL puts
not_dir_entry:

    ; print newline
0x00043D2A       LI R1 newline_str
0x00043D32       BL puts

0x00043D3A       B read_dir_loop

read_done:
    ; close(fd)
0x00043D42       MOV R1 R11
0x00043D46       BL close

0x00043D4E       ADD R10 R10 1           ; next directory
0x00043D52       B dir_loop

open_failed:
    ; print error message for this directory
0x00043D5A       LI R1 error_prefix
0x00043D62       BL puts
    ; print the directory name
0x00043D6A       MOV R2 R10
0x00043D6E       SHL R2 R2 2
0x00043D72       ADD R2 R9 R2
0x00043D76       LDW R1 [R2]
0x00043D7A       BL puts
0x00043D82       LI R1 newline_str
0x00043D8A       BL puts

0x00043D92       LI R6 1                 ; set return code to error
0x00043D9A       ADD R10 R10 1           ; next directory
0x00043D9E       B dir_loop

dir_done:
    ; free buffer
0x00043DA6       LI  R2 256
0x00043DAE       ADD SP SP R2

0x00043DB2       MOV R1 R6               ; return code
0x00043DB6       POP R12
0x00043DBA       POP R11
0x00043DBE       POP R10
0x00043DC2       POP R9
...
;==============================================================================
usage:
0x00043DDA       LI R1 usage_str
0x00043DE2       BL puts
0x00043DEA       LI R6 1                 ; error
0x00043DF2       B dir_done
