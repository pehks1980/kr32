...
0x00043FAE       PUSH R12
    ...
    ;DEBUG 2
0x00043FB2       LI  R3 256
0x00043FBA       SUB SP SP R3
0x00043FBE       MOV R12 SP              ; R12 = buffer pointer
...
0x0004405E       POP R1
0x00044062       LI  R2 O_RDONLY
0x0004406A       BL open
0x00044072       MOV R11 R1              ; R11 = fd

0x00044076       CMP R11 0
0x0004407A       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x00044082       MOV R1 R11
0x00044086       MOV R2 R12
0x0004408A       LI  R3 DIRENT_SIZEOF
0x00044092       BL read
0x0004409A       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x0004409E       CMP R7 0
0x000440A2       BEQ read_done           ; EOF
0x000440AA       CMP R7 DIRENT_SIZEOF
0x000440AE       BNE read_done           ; error or short read

    ; parse the directory entry
0x000440B6       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x000440BA       ADD R1 R12 DIRENT_NAME
0x000440BE       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x000440C6       CMP R5 DT_DIR
0x000440CA       BNE not_dir_entry
0x000440D2       LI R1 slash_str
0x000440DA       BL puts
not_dir_entry:

    ; print newline
0x000440E2       LI R1 newline_str
0x000440EA       BL puts

0x000440F2       B read_dir_loop
...
0x00044106       ADD R10 R10 1           ; next directory
0x0004410A       B dir_loop

open_failed:
    ; print error message for this directory
0x00044112       LI R1 error_prefix
0x0004411A       BL puts
    ; print the directory name
0x00044122       MOV R2 R10
0x00044126       SHL R2 R2 2
0x0004412A       ADD R2 R9 R2
0x0004412E       LDW R1 [R2]
0x00044132       BL puts
0x0004413A       LI R1 newline_str
0x00044142       BL puts

0x0004414A       LI R6 1                 ; set return code to error
0x00044152       ADD R10 R10 1           ; next directory
0x00044156       B dir_loop

dir_done:
    ; free buffer
0x0004415E       LI  R2 256
0x00044166       ADD SP SP R2
0x0004416A       MOV R1 R6               ; return code
0x0004416E       POP R12
0x00044172       POP R11
0x00044176       POP R10
0x0004417A       POP R9
...
;==============================================================================
usage:
0x00044192       LI R1 usage_str
0x0004419A       BL puts
0x000441A2       LI R6 1                 ; error
0x000441AA       B dir_done
