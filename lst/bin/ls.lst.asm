...
0x00043C2A       PUSH R12
    ...
    ;DEBUG 2
0x00043C2E       LI  R3 256
0x00043C36       SUB SP SP R3
0x00043C3A       MOV R12 SP              ; R12 = buffer pointer
...
0x00043CDA       POP R1
0x00043CDE       LI  R2 O_RDONLY
0x00043CE6       BL open
0x00043CEE       MOV R11 R1              ; R11 = fd

0x00043CF2       CMP R11 0
0x00043CF6       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x00043CFE       MOV R1 R11
0x00043D02       MOV R2 R12
0x00043D06       LI  R3 DIRENT_SIZEOF
0x00043D0E       BL read
0x00043D16       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043D1A       CMP R7 0
0x00043D1E       BEQ read_done           ; EOF
0x00043D26       CMP R7 DIRENT_SIZEOF
0x00043D2A       BNE read_done           ; error or short read

    ; parse the directory entry
0x00043D32       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00043D36       ADD R1 R12 DIRENT_NAME
0x00043D3A       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x00043D42       CMP R5 DT_DIR
0x00043D46       BNE not_dir_entry
0x00043D4E       LI R1 slash_str
0x00043D56       BL puts
not_dir_entry:

    ; print newline
0x00043D5E       LI R1 newline_str
0x00043D66       BL puts

0x00043D6E       B read_dir_loop

read_done:
    ; close(fd)
0x00043D76       MOV R1 R11
0x00043D7A       BL close

0x00043D82       ADD R10 R10 1           ; next directory
0x00043D86       B dir_loop

open_failed:
    ; print error message for this directory
0x00043D8E       LI R1 error_prefix
0x00043D96       BL puts
    ; print the directory name
0x00043D9E       MOV R2 R10
0x00043DA2       SHL R2 R2 2
0x00043DA6       ADD R2 R9 R2
0x00043DAA       LDW R1 [R2]
0x00043DAE       BL puts
0x00043DB6       LI R1 newline_str
0x00043DBE       BL puts

0x00043DC6       LI R6 1                 ; set return code to error
0x00043DCE       ADD R10 R10 1           ; next directory
0x00043DD2       B dir_loop

dir_done:
    ; free buffer
0x00043DDA       LI  R2 256
0x00043DE2       ADD SP SP R2
0x00043DE6       MOV R1 R6               ; return code
0x00043DEA       POP R12
0x00043DEE       POP R11
0x00043DF2       POP R10
0x00043DF6       POP R9
...
;==============================================================================
usage:
0x00043E0E       LI R1 usage_str
0x00043E16       BL puts
0x00043E1E       LI R6 1                 ; error
0x00043E26       B dir_done
