...
0x00043C26       LI  R3 256
0x00043C2E       SUB SP SP R3
0x00043C32       MOV R12 SP              ; R12 = buffer pointer
...
0x00043CD2       POP R1
0x00043CD6       LI  R2 O_RDONLY
0x00043CDE       BL open
0x00043CE6       MOV R11 R1              ; R11 = fd

0x00043CEA       CMP R11 0
0x00043CEE       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x00043CF6       MOV R1 R11
0x00043CFA       MOV R2 R12
0x00043CFE       LI  R3 DIRENT_SIZEOF
0x00043D06       BL read
0x00043D0E       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043D12       CMP R7 0
0x00043D16       BEQ read_done           ; EOF
0x00043D1E       CMP R7 DIRENT_SIZEOF
0x00043D22       BNE read_done           ; error or short read

    ; parse the directory entry
0x00043D2A       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00043D2E       ADD R1 R12 DIRENT_NAME
0x00043D32       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x00043D3A       CMP R5 DT_DIR
0x00043D3E       BNE not_dir_entry
0x00043D46       LI R1 slash_str
0x00043D4E       BL puts
not_dir_entry:

    ; print newline
0x00043D56       LI R1 newline_str
0x00043D5E       BL puts

0x00043D66       B read_dir_loop

read_done:
    ; close(fd)
0x00043D6E       MOV R1 R11
0x00043D72       BL close

0x00043D7A       ADD R10 R10 1           ; next directory
0x00043D7E       B dir_loop

open_failed:
    ; print error message for this directory
0x00043D86       LI R1 error_prefix
0x00043D8E       BL puts
    ; print the directory name
0x00043D96       MOV R2 R10
0x00043D9A       SHL R2 R2 2
0x00043D9E       ADD R2 R9 R2
0x00043DA2       LDW R1 [R2]
0x00043DA6       BL puts
0x00043DAE       LI R1 newline_str
0x00043DB6       BL puts

0x00043DBE       LI R6 1                 ; set return code to error
0x00043DC6       ADD R10 R10 1           ; next directory
0x00043DCA       B dir_loop

dir_done:
    ; free buffer
0x00043DD2       LI  R2 256
0x00043DDA       ADD SP SP R2

0x00043DDE       MOV R1 R6               ; return code
0x00043DE2       POP R12
0x00043DE6       POP R11
0x00043DEA       POP R10
0x00043DEE       POP R9
...
;==============================================================================
usage:
0x00043E06       LI R1 usage_str
0x00043E0E       BL puts
0x00043E16       LI R6 1                 ; error
0x00043E1E       B dir_done
