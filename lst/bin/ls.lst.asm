...
0x00044076       LI  R3 256
0x0004407E       SUB SP SP R3
0x00044082       MOV R12 SP              ; R12 = buffer pointer
...
0x00044122       POP R1
0x00044126       LI  R2 O_RDONLY
0x0004412E       BL open
0x00044136       MOV R11 R1              ; R11 = fd

0x0004413A       CMP R11 0
0x0004413E       BLT open_failed

read_dir_loop:
    ; read(fd, buf, DIRENT_SIZEOF)
0x00044146       MOV R1 R11
0x0004414A       MOV R2 R12
0x0004414E       LI  R3 DIRENT_SIZEOF
0x00044156       BL read
0x0004415E       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00044162       CMP R7 0
0x00044166       BEQ read_done           ; EOF
0x0004416E       CMP R7 DIRENT_SIZEOF
0x00044172       BNE read_done           ; error or short read

    ; parse the directory entry
0x0004417A       LDB R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x0004417E       ADD R1 R12 DIRENT_NAME
0x00044182       BL puts                  ; prints the name (libc puts writes string, no newline)

    ; if directory, print '/'
0x0004418A       CMP R5 DT_DIR
0x0004418E       BNE not_dir_entry
0x00044196       LI R1 slash_str
0x0004419E       BL puts
not_dir_entry:

    ; print newline
0x000441A6       LI R1 newline_str
0x000441AE       BL puts

0x000441B6       B read_dir_loop

read_done:
    ; close(fd)
0x000441BE       MOV R1 R11
0x000441C2       BL close

0x000441CA       ADD R10 R10 1           ; next directory
0x000441CE       B dir_loop

open_failed:
    ; print error message for this directory
0x000441D6       LI R1 error_prefix
0x000441DE       BL puts
    ; print the directory name
0x000441E6       MOV R2 R10
0x000441EA       SHL R2 R2 2
0x000441EE       ADD R2 R9 R2
0x000441F2       LDW R1 [R2]
0x000441F6       BL puts
0x000441FE       LI R1 newline_str
0x00044206       BL puts

0x0004420E       LI R6 1                 ; set return code to error
0x00044216       ADD R10 R10 1           ; next directory
0x0004421A       B dir_loop

dir_done:
    ; free buffer
0x00044222       LI  R2 256
0x0004422A       ADD SP SP R2
0x0004422E       MOV R1 R6               ; return code
0x00044232       POP R12
0x00044236       POP R11
0x0004423A       POP R10
0x0004423E       POP R9
...
;==============================================================================
usage:
0x00044256       LI R1 usage_str
0x0004425E       BL puts
0x00044266       LI R6 1                 ; error
0x0004426E       B dir_done
