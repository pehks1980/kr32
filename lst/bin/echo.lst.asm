...
0x000440B2       PUSH R8              ;
0x000440B6       PUSH R9              ;
0x000440BA       PUSH R10             ; Loop counter

0x000440BE       MOV R8 R1            ; R8 = argc
0x000440C2       MOV R9 R2            ; R9 = argv
0x000440C6       LI R10 1             ; Start from argv[1] (skip program name argv[0])
0x000440CE       MOV R11 R9           ; R11 = current argv pointer
0x000440D2       ADD R11 R11 4        ; Skip argv[0] (program name)

echo_next_arg:
0x000440D6       CMP R10 R8           ; Check if we've processed all arguments
0x000440DA       BGE echo_done

0x000440E2       LDW R1 [R11]         ; Load current argument string
0x000440E6       BL puts              ; Print the argument-string

0x000440EE       ADD R10 R10 1        ; Increment argument counter
0x000440F2       ADD R11 R11 4        ; Move to next argv entry
0x000440F6       CMP R10 R8           ; Check if this was the last argument
0x000440FA       BGE echo_newline     ; If last, just print newline
0x00044102       LI R1 space_str      ; Otherwise print space between arguments
0x0004410A       BL puts
0x00044112       B echo_next_arg

echo_newline:
0x0004411A       LI R1 newline_str    ; Print final newline
0x00044122       BL puts

echo_done:

0x0004412A       LI R1 0              ; Return success
0x00044132       POP R10
0x00044136       POP R9
0x0004413A       POP R8
0x0004413E       POP LR
