...
0x0004405E       PUSH R8              ;
0x00044062       PUSH R9              ;
0x00044066       PUSH R10             ; Loop counter

0x0004406A       MOV R8 R1            ; R8 = argc
0x0004406E       MOV R9 R2            ; R9 = argv
0x00044072       LI R10 1             ; Start from argv[1] (skip program name argv[0])
0x0004407A       MOV R11 R9           ; R11 = current argv pointer
0x0004407E       ADD R11 R11 4        ; Skip argv[0] (program name)

echo_next_arg:
0x00044082       CMP R10 R8           ; Check if we've processed all arguments
0x00044086       BGE echo_done

0x0004408E       LDW R1 [R11]         ; Load current argument string
0x00044092       BL puts              ; Print the argument-string

0x0004409A       ADD R10 R10 1        ; Increment argument counter
0x0004409E       ADD R11 R11 4        ; Move to next argv entry
0x000440A2       CMP R10 R8           ; Check if this was the last argument
0x000440A6       BGE echo_newline     ; If last, just print newline
0x000440AE       LI R1 space_str      ; Otherwise print space between arguments
0x000440B6       BL puts
0x000440BE       B echo_next_arg

echo_newline:
0x000440C6       LI R1 newline_str    ; Print final newline
0x000440CE       BL puts

echo_done:

0x000440D6       LI R1 0              ; Return success
0x000440DE       POP R10
0x000440E2       POP R9
0x000440E6       POP R8
0x000440EA       POP LR
