...
main:
0x00043BDA       NOP
0x00043BDE       DEBUG 2     ;testing INVLPG and tlb cache

0x00043BE2       PUSH LR
0x00043BE6       PUSH R8              ;
0x00043BEA       PUSH R9              ;
0x00043BEE       PUSH R10             ; Loop counter

0x00043BF2       MOV R8 R1            ; R8 = argc
0x00043BF6       MOV R9 R2            ; R9 = argv
0x00043BFA       LI R10 1             ; Start from argv[1] (skip program name argv[0])
0x00043C02       MOV R11 R9           ; R11 = current argv pointer
0x00043C06       ADD R11 R11 4        ; Skip argv[0] (program name)

echo_next_arg:
0x00043C0A       CMP R10 R8           ; Check if we've processed all arguments
0x00043C0E       BGE echo_done

0x00043C16       LDW R1 [R11]         ; Load current argument string
0x00043C1A       BL puts              ; Print the argument-string

0x00043C22       ADD R10 R10 1        ; Increment argument counter
0x00043C26       ADD R11 R11 4        ; Move to next argv entry
0x00043C2A       CMP R10 R8           ; Check if this was the last argument
0x00043C2E       BGE echo_newline     ; If last, just print newline
0x00043C36       LI R1 space_str      ; Otherwise print space between arguments
0x00043C3E       BL puts
0x00043C46       B echo_next_arg

echo_newline:
0x00043C4E       LI R1 newline_str    ; Print final newline
0x00043C56       BL puts

echo_done:

0x00043C5E       LI R1 0              ; Return success
0x00043C66       POP R10
0x00043C6A       POP R9
0x00043C6E       POP R8
0x00043C72       POP LR
