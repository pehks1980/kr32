...
main:
0x00043C06       NOP
0x00043C0A       DEBUG 2     ;testing INVLPG and tlb cache

0x00043C0E       PUSH LR
0x00043C12       PUSH R8              ;
0x00043C16       PUSH R9              ;
0x00043C1A       PUSH R10             ; Loop counter

0x00043C1E       MOV R8 R1            ; R8 = argc
0x00043C22       MOV R9 R2            ; R9 = argv
0x00043C26       LI R10 1             ; Start from argv[1] (skip program name argv[0])
0x00043C2E       MOV R11 R9           ; R11 = current argv pointer
0x00043C32       ADD R11 R11 4        ; Skip argv[0] (program name)

echo_next_arg:
0x00043C36       CMP R10 R8           ; Check if we've processed all arguments
0x00043C3A       BGE echo_done

0x00043C42       LDW R1 [R11]         ; Load current argument string
0x00043C46       BL puts              ; Print the argument-string

0x00043C4E       ADD R10 R10 1        ; Increment argument counter
0x00043C52       ADD R11 R11 4        ; Move to next argv entry
0x00043C56       CMP R10 R8           ; Check if this was the last argument
0x00043C5A       BGE echo_newline     ; If last, just print newline
0x00043C62       LI R1 space_str      ; Otherwise print space between arguments
0x00043C6A       BL puts
0x00043C72       B echo_next_arg

echo_newline:
0x00043C7A       LI R1 newline_str    ; Print final newline
0x00043C82       BL puts

echo_done:

0x00043C8A       LI R1 0              ; Return success
0x00043C92       POP R10
0x00043C96       POP R9
0x00043C9A       POP R8
0x00043C9E       POP LR
