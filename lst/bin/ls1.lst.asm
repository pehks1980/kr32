...
0x00043FB2       LI  R3 DIRENT_SIZEOF
0x00043FBA       SUB SP SP R3
0x00043FBE       MOV R12 SP              ; R12 = pointer to struct dirent buffer

0x00043FC2       MOV R8 R1               ; R8 = argc
0x00043FC6       MOV R9 R2               ; R9 = argv

0x00043FCA       CMP R8 2                ; Need at least one argument (argv[1])
0x00043FCE       BLT usage



0x00043FD6       LI R10 1                ; R10 = current argument index (argv[1])
0x00043FDE       LI R6 0                 ; R6 = return code (0 = success)

dir_loop:
0x00043FE6       CMP R10 R8              ; if index >= argc, done
0x00043FEA       BGE dir_done

    ; Get the path string from argv[index]
0x00043FF2       MOV R2 R10
0x00043FF6       SHL R2 R2 2
0x00043FFA       ADD R2 R9 R2
0x00043FFE       LDW R1 [R2]             ; R1 = directory path (e.g., "etc/")
0x00044002       PUSH R1

    ; Print header: "\n--- Directory: path ---\n"
0x00044006       LI R1 newline_str
0x0004400E   CALL puts
0x00044016       LI R1 dir_header_prefix
0x0004401E   CALL puts
    ; print the directory name
0x00044026       MOV R2 R10
0x0004402A       SHL R2 R2 2
0x0004402E       ADD R2 R9 R2
0x00044032       LDW R1 [R2]
0x00044036   CALL puts
0x0004403E       LI R1 dir_header_suffix
0x00044046   CALL puts
0x0004404E       LI R1 newline_str
0x00044056   CALL puts

    ; open directory using opendir wrapper
0x0004405E       POP R1                  ; path
0x00044062   CALL opendir

0x0004406A       MOV R11 R1              ; R11 = DIR* handle

0x0004406E       CMP R11 0
0x00044072       BEQ open_failed         ; opendir returns 0 on error

read_dir_loop:
    ; Read next directory entry
0x0004407A       MOV R1 R11              ; DIR*
0x0004407E       MOV R2 R12              ; pointer to dirent buffer
0x00044082   CALL readdir
0x0004408A       CMP R1 0
0x0004408E       BEQ read_done           ; EOF
0x00044096       LI  R2 -1
0x0004409E       CMP R1 R2
0x000440A2       BEQ read_done           ; error

    ; parse the directory entry
0x000440AA       LDW R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x000440AE       ADD R1 R12 DIRENT_NAME
0x000440B2   CALL puts

    ; if directory, print '/'
0x000440BA       CMP R5 DT_DIR
0x000440BE       BNE not_dir_entry
0x000440C6       LI R1 slash_str
0x000440CE   CALL puts
not_dir_entry:

    ; print newline
0x000440D6       LI R1 newline_str
0x000440DE   CALL puts

0x000440E6       B read_dir_loop

read_done:
    ; close directory using closedir wrapper
0x000440EE       MOV R1 R11
0x000440F2   CALL closedir

0x000440FA       ADD R10 R10 1           ; next directory
0x000440FE       B dir_loop

open_failed:
    ; print error message for this directory
0x00044106       LI R1 error_prefix
0x0004410E   CALL puts
    ; print the directory name
0x00044116       MOV R2 R10
0x0004411A       SHL R2 R2 2
0x0004411E       ADD R2 R9 R2
0x00044122       LDW R1 [R2]
0x00044126   CALL puts
0x0004412E       LI R1 ls_newline_str
0x00044136   CALL puts

0x0004413E       LI R6 1                 ; set return code to error
0x00044146       ADD R10 R10 1           ; next directory
0x0004414A       B dir_loop

dir_done:
    ; free buffer
0x00044152       LI  R3 DIRENT_SIZEOF
0x0004415A       ADD SP SP R3

0x0004415E       MOV R1 R6               ; return code
0x00044162       POP R12
0x00044166       POP R11
0x0004416A       POP R10
0x0004416E       POP R9
...
;==============================================================================
usage:
0x00044186       LI R1 usage_str
0x0004418E   CALL puts
0x00044196       LI R6 1                 ; error
0x0004419E       B dir_done
