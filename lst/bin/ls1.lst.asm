...
0x00044076       LI  R3 DIRENT_SIZEOF
0x0004407E       SUB SP SP R3
0x00044082       MOV R12 SP              ; R12 = pointer to struct dirent buffer

0x00044086       MOV R8 R1               ; R8 = argc
0x0004408A       MOV R9 R2               ; R9 = argv

0x0004408E       CMP R8 2                ; Need at least one argument (argv[1])
0x00044092       BLT usage



0x0004409A       LI R10 1                ; R10 = current argument index (argv[1])
0x000440A2       LI R6 0                 ; R6 = return code (0 = success)

dir_loop:
0x000440AA       CMP R10 R8              ; if index >= argc, done
0x000440AE       BGE dir_done

    ; Get the path string from argv[index]
0x000440B6       MOV R2 R10
0x000440BA       SHL R2 R2 2
0x000440BE       ADD R2 R9 R2
0x000440C2       LDW R1 [R2]             ; R1 = directory path (e.g., "etc/")
0x000440C6       PUSH R1

    ; Print header: "\n--- Directory: path ---\n"
0x000440CA       LI R1 newline_str
0x000440D2   CALL puts
0x000440DA       LI R1 dir_header_prefix
0x000440E2   CALL puts
    ; print the directory name
0x000440EA       MOV R2 R10
0x000440EE       SHL R2 R2 2
0x000440F2       ADD R2 R9 R2
0x000440F6       LDW R1 [R2]
0x000440FA   CALL puts
0x00044102       LI R1 dir_header_suffix
0x0004410A   CALL puts
0x00044112       LI R1 newline_str
0x0004411A   CALL puts

    ; open directory using opendir wrapper
0x00044122       POP R1                  ; path
0x00044126   CALL opendir

0x0004412E       MOV R11 R1              ; R11 = DIR* handle

0x00044132       CMP R11 0
0x00044136       BEQ open_failed         ; opendir returns 0 on error

read_dir_loop:
    ; Read next directory entry
0x0004413E       MOV R1 R11              ; DIR*
0x00044142       MOV R2 R12              ; pointer to dirent buffer
0x00044146   CALL readdir
0x0004414E       CMP R1 0
0x00044152       BEQ read_done           ; EOF
0x0004415A       LI  R2 -1
0x00044162       CMP R1 R2
0x00044166       BEQ read_done           ; error

    ; parse the directory entry
0x0004416E       LDW R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00044172       ADD R1 R12 DIRENT_NAME
0x00044176   CALL puts

    ; if directory, print '/'
0x0004417E       CMP R5 DT_DIR
0x00044182       BNE not_dir_entry
0x0004418A       LI R1 slash_str
0x00044192   CALL puts
not_dir_entry:

    ; print newline
0x0004419A       LI R1 newline_str
0x000441A2   CALL puts

0x000441AA       B read_dir_loop

read_done:
    ; close directory using closedir wrapper
0x000441B2       MOV R1 R11
0x000441B6   CALL closedir

0x000441BE       ADD R10 R10 1           ; next directory
0x000441C2       B dir_loop

open_failed:
    ; print error message for this directory
0x000441CA       LI R1 error_prefix
0x000441D2   CALL puts
    ; print the directory name
0x000441DA       MOV R2 R10
0x000441DE       SHL R2 R2 2
0x000441E2       ADD R2 R9 R2
0x000441E6       LDW R1 [R2]
0x000441EA   CALL puts
0x000441F2       LI R1 ls_newline_str
0x000441FA   CALL puts

0x00044202       LI R6 1                 ; set return code to error
0x0004420A       ADD R10 R10 1           ; next directory
0x0004420E       B dir_loop

dir_done:
    ; free buffer
0x00044216       LI  R3 DIRENT_SIZEOF
0x0004421E       ADD SP SP R3

0x00044222       MOV R1 R6               ; return code
0x00044226       POP R12
0x0004422A       POP R11
0x0004422E       POP R10
0x00044232       POP R9
...
;==============================================================================
usage:
0x0004424A       LI R1 usage_str
0x00044252   CALL puts
0x0004425A       LI R6 1                 ; error
0x00044262       B dir_done
