...
0x00043C12       LI  R3 DIRENT_SIZEOF
0x00043C1A       SUB SP SP R3
0x00043C1E       MOV R12 SP              ; R12 = pointer to struct dirent buffer
...
0x00043C96   CALL puts
0x00043C9E       LI R1 dir_header_suffix
0x00043CA6   CALL puts
0x00043CAE       LI R1 newline_str
0x00043CB6   CALL puts

    ; open directory using opendir wrapper
0x00043CBE       POP R1                  ; path
0x00043CC2   CALL opendir

0x00043CCA       MOV R11 R1              ; R11 = DIR* handle

0x00043CCE       CMP R11 0
0x00043CD2       BEQ open_failed         ; opendir returns 0 on error

read_dir_loop:
    ; Read next directory entry
0x00043CDA       MOV R1 R11              ; DIR*
0x00043CDE       MOV R2 R12              ; pointer to dirent buffer
0x00043CE2   CALL readdir
0x00043CEA       CMP R1 0
0x00043CEE       BEQ read_done           ; EOF
0x00043CF6       LI  R2 -1
0x00043CFE       CMP R1 R2
0x00043D02       BEQ read_done           ; error

    ; parse the directory entry
0x00043D0A       LDW R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00043D0E       ADD R1 R12 DIRENT_NAME
0x00043D12   CALL puts

    ; if directory, print '/'
0x00043D1A       CMP R5 DT_DIR
0x00043D1E       BNE not_dir_entry
0x00043D26       LI R1 slash_str
0x00043D2E   CALL puts
not_dir_entry:

    ; print newline
0x00043D36       LI R1 newline_str
0x00043D3E   CALL puts

0x00043D46       B read_dir_loop

read_done:
    ; close directory using closedir wrapper
0x00043D4E       MOV R1 R11
0x00043D52   CALL closedir

0x00043D5A       ADD R10 R10 1           ; next directory
0x00043D5E       B dir_loop

open_failed:
    ; print error message for this directory
0x00043D66       LI R1 error_prefix
0x00043D6E   CALL puts
    ; print the directory name
0x00043D76       MOV R2 R10
0x00043D7A       SHL R2 R2 2
0x00043D7E       ADD R2 R9 R2
0x00043D82       LDW R1 [R2]
0x00043D86   CALL puts
0x00043D8E       LI R1 ls_newline_str
0x00043D96   CALL puts

0x00043D9E       LI R6 1                 ; set return code to error
0x00043DA6       ADD R10 R10 1           ; next directory
0x00043DAA       B dir_loop

dir_done:
    ; free buffer
0x00043DB2       LI  R3 DIRENT_SIZEOF
0x00043DBA       ADD SP SP R3

0x00043DBE       MOV R1 R6               ; return code
0x00043DC2       POP R12
0x00043DC6       POP R11
0x00043DCA       POP R10
0x00043DCE       POP R9
...
;==============================================================================
usage:
0x00043DE6       LI R1 usage_str
0x00043DEE   CALL puts
0x00043DF6       LI R6 1                 ; error
0x00043DFE       B dir_done
