...
0x00043C1A       LI  R3 DIRENT_SIZEOF
0x00043C22       SUB SP SP R3
0x00043C26       MOV R12 SP              ; R12 = pointer to struct dirent buffer

0x00043C2A       MOV R8 R1               ; R8 = argc
0x00043C2E       MOV R9 R2               ; R9 = argv

0x00043C32       CMP R8 2                ; Need at least one argument (argv[1])
0x00043C36       BLT usage



0x00043C3E       LI R10 1                ; R10 = current argument index (argv[1])
0x00043C46       LI R6 0                 ; R6 = return code (0 = success)

dir_loop:
0x00043C4E       CMP R10 R8              ; if index >= argc, done
0x00043C52       BGE dir_done

    ; Get the path string from argv[index]
0x00043C5A       MOV R2 R10
0x00043C5E       SHL R2 R2 2
0x00043C62       ADD R2 R9 R2
0x00043C66       LDW R1 [R2]             ; R1 = directory path (e.g., "etc/")
0x00043C6A       PUSH R1

    ; Print header: "\n--- Directory: path ---\n"
0x00043C6E       LI R1 newline_str
0x00043C76   CALL puts
0x00043C7E       LI R1 dir_header_prefix
0x00043C86   CALL puts
    ; print the directory name
0x00043C8E       MOV R2 R10
0x00043C92       SHL R2 R2 2
0x00043C96       ADD R2 R9 R2
0x00043C9A       LDW R1 [R2]
0x00043C9E   CALL puts
0x00043CA6       LI R1 dir_header_suffix
0x00043CAE   CALL puts
0x00043CB6       LI R1 newline_str
0x00043CBE   CALL puts

    ; open directory using opendir wrapper
0x00043CC6       POP R1                  ; path
0x00043CCA   CALL opendir

0x00043CD2       MOV R11 R1              ; R11 = DIR* handle

0x00043CD6       CMP R11 0
0x00043CDA       BEQ open_failed         ; opendir returns 0 on error

read_dir_loop:
    ; Read next directory entry
0x00043CE2       MOV R1 R11              ; DIR*
0x00043CE6       MOV R2 R12              ; pointer to dirent buffer
0x00043CEA   CALL readdir
0x00043CF2       CMP R1 0
0x00043CF6       BEQ read_done           ; EOF
0x00043CFE       LI  R2 -1
0x00043D06       CMP R1 R2
0x00043D0A       BEQ read_done           ; error

    ; parse the directory entry
0x00043D12       LDW R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x00043D16       ADD R1 R12 DIRENT_NAME
0x00043D1A   CALL puts

    ; if directory, print '/'
0x00043D22       CMP R5 DT_DIR
0x00043D26       BNE not_dir_entry
0x00043D2E       LI R1 slash_str
0x00043D36   CALL puts
not_dir_entry:

    ; print newline
0x00043D3E       LI R1 newline_str
0x00043D46   CALL puts

0x00043D4E       B read_dir_loop

read_done:
    ; close directory using closedir wrapper
0x00043D56       MOV R1 R11
0x00043D5A   CALL closedir

0x00043D62       ADD R10 R10 1           ; next directory
0x00043D66       B dir_loop

open_failed:
    ; print error message for this directory
0x00043D6E       LI R1 error_prefix
0x00043D76   CALL puts
    ; print the directory name
0x00043D7E       MOV R2 R10
0x00043D82       SHL R2 R2 2
0x00043D86       ADD R2 R9 R2
0x00043D8A       LDW R1 [R2]
0x00043D8E   CALL puts
0x00043D96       LI R1 ls_newline_str
0x00043D9E   CALL puts

0x00043DA6       LI R6 1                 ; set return code to error
0x00043DAE       ADD R10 R10 1           ; next directory
0x00043DB2       B dir_loop

dir_done:
    ; free buffer
0x00043DBA       LI  R3 DIRENT_SIZEOF
0x00043DC2       ADD SP SP R3

0x00043DC6       MOV R1 R6               ; return code
0x00043DCA       POP R12
0x00043DCE       POP R11
0x00043DD2       POP R10
0x00043DD6       POP R9
...
;==============================================================================
usage:
0x00043DEE       LI R1 usage_str
0x00043DF6   CALL puts
0x00043DFE       LI R6 1                 ; error
0x00043E06       B dir_done
