...
0x000440BE       LI  R3 DIRENT_SIZEOF
0x000440C6       SUB SP SP R3
0x000440CA       MOV R12 SP              ; R12 = pointer to struct dirent buffer
...
0x000440F6       BGE dir_done

    ; Get the path string from argv[index]
0x000440FE       MOV R2 R10
0x00044102       SHL R2 R2 2
0x00044106       ADD R2 R9 R2
0x0004410A       LDW R1 [R2]             ; R1 = directory path (e.g., "etc/")
0x0004410E       PUSH R1

    ; Print header: "\n--- Directory: path ---\n"
0x00044112       LI R1 newline_str
0x0004411A   CALL puts
0x00044122       LI R1 dir_header_prefix
0x0004412A   CALL puts
    ; print the directory name
0x00044132       MOV R2 R10
0x00044136       SHL R2 R2 2
0x0004413A       ADD R2 R9 R2
0x0004413E       LDW R1 [R2]
0x00044142   CALL puts
0x0004414A       LI R1 dir_header_suffix
0x00044152   CALL puts
0x0004415A       LI R1 newline_str
0x00044162   CALL puts

    ; open directory using opendir wrapper
0x0004416A       POP R1                  ; path
0x0004416E   CALL opendir

0x00044176       MOV R11 R1              ; R11 = DIR* handle

0x0004417A       CMP R11 0
0x0004417E       BEQ open_failed         ; opendir returns 0 on error

read_dir_loop:
    ; Read next directory entry
0x00044186       MOV R1 R11              ; DIR*
0x0004418A       MOV R2 R12              ; pointer to dirent buffer
0x0004418E   CALL readdir
0x00044196       CMP R1 0
0x0004419A       BEQ read_done           ; EOF
0x000441A2       LI  R2 -1
0x000441AA       CMP R1 R2
0x000441AE       BEQ read_done           ; error

    ; parse the directory entry
0x000441B6       LDW R5 [R12 + DIRENT_TYPE]   ; R5 = d_type (DT_REG or DT_DIR)

    ; print filename (null-terminated at R12 + DIRENT_NAME)
0x000441BA       ADD R1 R12 DIRENT_NAME
0x000441BE   CALL puts

    ; if directory, print '/'
0x000441C6       CMP R5 DT_DIR
0x000441CA       BNE not_dir_entry
0x000441D2       LI R1 slash_str
0x000441DA   CALL puts
not_dir_entry:
