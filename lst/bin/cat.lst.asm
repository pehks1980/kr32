...
0x000440BE       LI R3 256
0x000440C6       SUB SP SP R3
0x000440CA       MOV R12 SP              ; R12 = buffer pointer

0x000440CE       MOV R8 R1               ; R8 = argc
0x000440D2       MOV R9 R2               ; R9 = argv

0x000440D6       CMP R8 2                ; Need at least one argument (argv[1])

0x000440DA       BLT usage

0x000440E2       LI R10 1                ; R10 = current argument index (argv[1])
0x000440EA       LI R6 0                 ; R6 = return code (0 = success)

file_loop:
0x000440F2       CMP R10 R8              ; if index >= argc, done
0x000440F6       BGE file_done

    ; open(argv[index], O_RDONLY)
0x000440FE       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x00044102       SHL R2 R2 2
0x00044106       ADD R2 R9 R2            ; R2 = address of argv[index]
0x0004410A       LDW R1 [R2]             ; R1 = filename
0x0004410E       LI R2 0                 ; O_RDONLY
0x00044116       BL open
0x0004411E       MOV R11 R1              ; R11 = fd

0x00044122       CMP R11 0               ; if fd < 0, error
0x00044126       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x0004412E       MOV R1 R11
0x00044132       MOV R2 R12
0x00044136       LI R3 256
0x0004413E       BL read
0x00044146       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x0004414A       CMP R7 0
0x0004414E       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x00044156       LI R1 1                 ; stdout
0x0004415E       MOV R2 R12
0x00044162       MOV R3 R7
0x00044166       BL write

0x0004416E       B read_loop

read_done:
    ; close(fd)
0x00044176       MOV R1 R11
0x0004417A       BL close

0x00044182       ADD R10 R10 1           ; next file
0x00044186       B file_loop

open_failed:
    ; print error message for this file
0x0004418E       LI R1 error_prefix
0x00044196       BL puts
    ; print the filename
0x0004419E       MOV R2 R10
0x000441A2       SHL R2 R2 2
0x000441A6       ADD R2 R9 R2
0x000441AA       LDW R1 [R2]
0x000441AE       BL puts
0x000441B6       LI R1 newline_str
0x000441BE       BL puts

0x000441C6       LI R6 1                 ; set return code to error
0x000441CE       ADD R10 R10 1           ; next file
0x000441D2       B file_loop

file_done:
    ; free buffer
0x000441DA       LI  R2 256
0x000441E2       ADD SP SP R2
0x000441E6       MOV R1 R6               ; return code
0x000441EA       POP R12
0x000441EE       POP R11
0x000441F2       POP R10
0x000441F6       POP R9
...
usage:
0x0004420E       LI R1 cat_usage_str
0x00044216       BL puts
0x0004421E       LI R6 1                 ; error
0x00044226       B file_done
