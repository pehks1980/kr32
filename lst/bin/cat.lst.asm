...
0x00043C1E       LI R3 256
0x00043C26       SUB SP SP R3
0x00043C2A       MOV R12 SP              ; R12 = buffer pointer

0x00043C2E       MOV R8 R1               ; R8 = argc
0x00043C32       MOV R9 R2               ; R9 = argv

0x00043C36       CMP R8 2                ; Need at least one argument (argv[1])

0x00043C3A       BLT usage

0x00043C42       LI R10 1                ; R10 = current argument index (argv[1])
0x00043C4A       LI R6 0                 ; R6 = return code (0 = success)

file_loop:
0x00043C52       CMP R10 R8              ; if index >= argc, done
0x00043C56       BGE file_done

    ; open(argv[index], O_RDONLY)
0x00043C5E       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x00043C62       SHL R2 R2 2
0x00043C66       ADD R2 R9 R2            ; R2 = address of argv[index]
0x00043C6A       LDW R1 [R2]             ; R1 = filename
0x00043C6E       LI R2 0                 ; O_RDONLY
0x00043C76       BL open
0x00043C7E       MOV R11 R1              ; R11 = fd

0x00043C82       CMP R11 0               ; if fd < 0, error
0x00043C86       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x00043C8E       MOV R1 R11
0x00043C92       MOV R2 R12
0x00043C96       LI R3 256
0x00043C9E       BL read
0x00043CA6       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043CAA       CMP R7 0
0x00043CAE       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x00043CB6       LI R1 1                 ; stdout
0x00043CBE       MOV R2 R12
0x00043CC2       MOV R3 R7
0x00043CC6       BL write

0x00043CCE       B read_loop

read_done:
    ; close(fd)
0x00043CD6       MOV R1 R11
0x00043CDA       BL close

0x00043CE2       ADD R10 R10 1           ; next file
0x00043CE6       B file_loop

open_failed:
    ; print error message for this file
0x00043CEE       LI R1 error_prefix
0x00043CF6       BL puts
    ; print the filename
0x00043CFE       MOV R2 R10
0x00043D02       SHL R2 R2 2
0x00043D06       ADD R2 R9 R2
0x00043D0A       LDW R1 [R2]
0x00043D0E       BL puts
0x00043D16       LI R1 newline_str
0x00043D1E       BL puts

0x00043D26       LI R6 1                 ; set return code to error
0x00043D2E       ADD R10 R10 1           ; next file
0x00043D32       B file_loop

file_done:
0x00043D3A       DEBUG 2                 ; inspect stack/register state before epilogue
    ; free buffer
0x00043D3E       LI  R2 256
0x00043D46       ADD SP SP R2

0x00043D4A       DEBUG 2                 ; inspect stack/register state after buffer cleanup
0x00043D4E       MOV R1 R6               ; return code
0x00043D52       POP R12
0x00043D56       POP R11
0x00043D5A       POP R10
0x00043D5E       POP R9
...
usage:
0x00043D76       DEBUG 2                 ; usage path should also preserve stack cleanly
0x00043D7A       LI R1 cat_usage_str
0x00043D82       BL puts
0x00043D8A       LI R6 1                 ; error
0x00043D92       B file_done
