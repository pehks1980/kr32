...
0x00043FEA       BGE file_done

    ; open(argv[index], O_RDONLY)
0x00043FF2       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x00043FF6       SHL R2 R2 2
0x00043FFA       ADD R2 R9 R2            ; R2 = address of argv[index]
0x00043FFE       LDW R1 [R2]             ; R1 = filename
0x00044002       LI R2 0                 ; O_RDONLY
0x0004400A       BL open
0x00044012       MOV R11 R1              ; R11 = fd

0x00044016       CMP R11 0               ; if fd < 0, error
0x0004401A       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x00044022       MOV R1 R11
0x00044026       MOV R2 R12
0x0004402A       LI R3 256
0x00044032       BL read
0x0004403A       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x0004403E       CMP R7 0
0x00044042       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x0004404A       LI R1 1                 ; stdout
0x00044052       MOV R2 R12
0x00044056       MOV R3 R7
0x0004405A       BL write

0x00044062       B read_loop

read_done:
    ; close(fd)
0x0004406A       MOV R1 R11
0x0004406E       BL close

0x00044076       ADD R10 R10 1           ; next file
0x0004407A       B file_loop

open_failed:
    ; print error message for this file
0x00044082       LI R1 error_prefix
0x0004408A       BL puts
    ; print the filename
0x00044092       MOV R2 R10
0x00044096       SHL R2 R2 2
0x0004409A       ADD R2 R9 R2
0x0004409E       LDW R1 [R2]
0x000440A2       BL puts
0x000440AA       LI R1 newline_str
0x000440B2       BL puts

0x000440BA       LI R6 1                 ; set return code to error
0x000440C2       ADD R10 R10 1           ; next file
0x000440C6       B file_loop

file_done:
    ; free buffer
0x000440CE       LI  R2 256
0x000440D6       ADD SP SP R2
0x000440DA       MOV R1 R6               ; return code
0x000440DE       POP R12
0x000440E2       POP R11
0x000440E6       POP R10
0x000440EA       POP R9
...
usage:
0x00044102       LI R1 cat_usage_str
0x0004410A       BL puts
0x00044112       LI R6 1                 ; error
0x0004411A       B file_done
