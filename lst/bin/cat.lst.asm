...
0x000440AE       BGE file_done

    ; open(argv[index], O_RDONLY)
0x000440B6       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x000440BA       SHL R2 R2 2
0x000440BE       ADD R2 R9 R2            ; R2 = address of argv[index]
0x000440C2       LDW R1 [R2]             ; R1 = filename
0x000440C6       LI R2 0                 ; O_RDONLY
0x000440CE       BL open
0x000440D6       MOV R11 R1              ; R11 = fd

0x000440DA       CMP R11 0               ; if fd < 0, error
0x000440DE       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x000440E6       MOV R1 R11
0x000440EA       MOV R2 R12
0x000440EE       LI R3 256
0x000440F6       BL read
0x000440FE       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00044102       CMP R7 0
0x00044106       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x0004410E       LI R1 1                 ; stdout
0x00044116       MOV R2 R12
0x0004411A       MOV R3 R7
0x0004411E       BL write

0x00044126       B read_loop

read_done:
    ; close(fd)
0x0004412E       MOV R1 R11
0x00044132       BL close

0x0004413A       ADD R10 R10 1           ; next file
0x0004413E       B file_loop

open_failed:
    ; print error message for this file
0x00044146       LI R1 error_prefix
0x0004414E       BL puts
    ; print the filename
0x00044156       MOV R2 R10
0x0004415A       SHL R2 R2 2
0x0004415E       ADD R2 R9 R2
0x00044162       LDW R1 [R2]
0x00044166       BL puts
0x0004416E       LI R1 newline_str
0x00044176       BL puts

0x0004417E       LI R6 1                 ; set return code to error
0x00044186       ADD R10 R10 1           ; next file
0x0004418A       B file_loop

file_done:
    ; free buffer
0x00044192       LI  R2 256
0x0004419A       ADD SP SP R2
0x0004419E       MOV R1 R6               ; return code
0x000441A2       POP R12
0x000441A6       POP R11
0x000441AA       POP R10
0x000441AE       POP R9
...
usage:
0x000441C6       LI R1 cat_usage_str
0x000441CE       BL puts
0x000441D6       LI R6 1                 ; error
0x000441DE       B file_done
