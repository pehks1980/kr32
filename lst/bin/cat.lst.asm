...
0x00043C5E       BGE file_done

    ; open(argv[index], O_RDONLY)
0x00043C66       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x00043C6A       SHL R2 R2 2
0x00043C6E       ADD R2 R9 R2            ; R2 = address of argv[index]
0x00043C72       LDW R1 [R2]             ; R1 = filename
0x00043C76       LI R2 0                 ; O_RDONLY
0x00043C7E       BL open
0x00043C86       MOV R11 R1              ; R11 = fd

0x00043C8A       CMP R11 0               ; if fd < 0, error
0x00043C8E       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x00043C96       MOV R1 R11
0x00043C9A       MOV R2 R12
0x00043C9E       LI R3 256
0x00043CA6       BL read
0x00043CAE       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043CB2       CMP R7 0
0x00043CB6       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x00043CBE       LI R1 1                 ; stdout
0x00043CC6       MOV R2 R12
0x00043CCA       MOV R3 R7
0x00043CCE       BL write

0x00043CD6       B read_loop

read_done:
    ; close(fd)
0x00043CDE       MOV R1 R11
0x00043CE2       BL close

0x00043CEA       ADD R10 R10 1           ; next file
0x00043CEE       B file_loop

open_failed:
    ; print error message for this file
0x00043CF6       LI R1 error_prefix
0x00043CFE       BL puts
    ; print the filename
0x00043D06       MOV R2 R10
0x00043D0A       SHL R2 R2 2
0x00043D0E       ADD R2 R9 R2
0x00043D12       LDW R1 [R2]
0x00043D16       BL puts
0x00043D1E       LI R1 newline_str
0x00043D26       BL puts

0x00043D2E       LI R6 1                 ; set return code to error
0x00043D36       ADD R10 R10 1           ; next file
0x00043D3A       B file_loop

file_done:
    ; free buffer
0x00043D42       LI  R2 256
0x00043D4A       ADD SP SP R2
...
usage:
0x00043D76       LI R1 cat_usage_str
0x00043D7E       BL puts
0x00043D86       LI R6 1                 ; error
0x00043D8E       B file_done
