...
0x00043C66       BGE file_done

    ; open(argv[index], O_RDONLY)
0x00043C6E       MOV R2 R10               ; R2 = index * 4 (since argv is an array of pointers)
0x00043C72       SHL R2 R2 2
0x00043C76       ADD R2 R9 R2            ; R2 = address of argv[index]
0x00043C7A       LDW R1 [R2]             ; R1 = filename
0x00043C7E       LI R2 0                 ; O_RDONLY
0x00043C86       BL open
0x00043C8E       MOV R11 R1              ; R11 = fd

0x00043C92       CMP R11 0               ; if fd < 0, error
0x00043C96       BLT open_failed

read_loop:
    ; read(fd, buf, 256)
0x00043C9E       MOV R1 R11
0x00043CA2       MOV R2 R12
0x00043CA6       LI R3 256
0x00043CAE       BL read
0x00043CB6       MOV R7 R1               ; R7 = bytes read (or -1 on error)

0x00043CBA       CMP R7 0
0x00043CBE       BLE read_done           ; if <= 0, done (EOF or error)

    ; write(1, buf, n)
0x00043CC6       LI R1 1                 ; stdout
0x00043CCE       MOV R2 R12
0x00043CD2       MOV R3 R7
0x00043CD6       BL write

0x00043CDE       B read_loop

read_done:
    ; close(fd)
0x00043CE6       MOV R1 R11
0x00043CEA       BL close

0x00043CF2       ADD R10 R10 1           ; next file
0x00043CF6       B file_loop

open_failed:
    ; print error message for this file
0x00043CFE       LI R1 error_prefix
0x00043D06       BL puts
    ; print the filename
0x00043D0E       MOV R2 R10
0x00043D12       SHL R2 R2 2
0x00043D16       ADD R2 R9 R2
0x00043D1A       LDW R1 [R2]
0x00043D1E       BL puts
0x00043D26       LI R1 newline_str
0x00043D2E       BL puts

0x00043D36       LI R6 1                 ; set return code to error
0x00043D3E       ADD R10 R10 1           ; next file
0x00043D42       B file_loop

file_done:
    ; free buffer
0x00043D4A       LI  R2 256
0x00043D52       ADD SP SP R2
0x00043D56       MOV R1 R6               ; return code
0x00043D5A       POP R12
0x00043D5E       POP R11
0x00043D62       POP R10
0x00043D66       POP R9
...
usage:
0x00043D7E       LI R1 cat_usage_str
0x00043D86       BL puts
0x00043D8E       LI R6 1                 ; error
0x00043D96       B file_done
